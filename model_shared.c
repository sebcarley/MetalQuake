...
static void Mod_GenerateLightmaps(model_t *model)
{
	...
	// timing so a long ray-traced bake is not mistaken for a hang, and so the
	// lightmap and lightgrid phases (the parallelization targets) can be measured
	double t_start, t_lm, t_grid, t_end;
	loadmodel = model;

	Con_Printf("Generating lightmaps for \"%s\" (ray-traced, this can take a while)...\n", model->name);
	t_start = Sys_DirtyTime();
	...
	Con_Printf("Lightmap generation for \"%s\" took %.2f sec (lightmaps %.2fs, lightgrid %.2fs).\n",
		model->name, max(0.0, t_end - t_start), max(0.0, t_grid - t_lm), max(0.0, t_end - t_grid));
