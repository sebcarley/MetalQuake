...
typedef enum shadermode_e
{
	...
	SHADERMODE_VOLUMETRICFOG, ///< (volumetric) raymarch the murk from reconstructed scene depth
	SHADERMODE_RTCOMPOSITE, ///< (Metal only) multiply the reprojected RT term into the scene
	SHADERMODE_M5BOLT, ///< (Metal only) accumulate the thunderbolt as capsule SDFs under max()
	SHADERMODE_MOTIONFILL, ///< (Metal only) camera-only screen-space motion vectors from scene depth, for MetalFX temporal
	SHADERMODE_MOTIONVECTOR, ///< (Metal only) per-entity motion vectors: current and previous model matrices, drawn over the fill
	SHADERMODE_REACTIVESTAMP, ///< (Metal only) the particles' own screen footprint stamped into the temporal scaler's reactive mask
	SHADERMODE_BLOOMBLUR, ///< BEAUTY A1 (2026-09-16): the modern bloom's prefilter, downsample and upsample passes (r_bloom_m5), both backends; the 2001 name, given its enum entry at last
	SHADERMODE_FXAAPOST, ///< (Metal only) FXAA at NATIVE resolution, after the MetalFX upscale -- the render-res postprocess antialiases a frame that is then magnified, so its FXAA cannot touch the staircase the magnification creates
	SHADERMODE_SMAAEDGES, ///< (Metal only) analytic MLAA pass 1: luma discontinuities on texel boundaries
	SHADERMODE_SMAAWEIGHTS, ///< (Metal only) analytic MLAA pass 2: walk each edge line to its ends and integrate the revectorised edge over each texel
	SHADERMODE_SMAABLEND, ///< (Metal only) analytic MLAA pass 3: colour by those weights -- FXAA corrects half a pixel near a step's ends, this blends every pixel of the step by its own coverage
	SHADERMODE_COUNT
}
shadermode_t;
...
/// METAL.md Phase 7. Declared here rather than duplicated as local externs in
/// vid_metal.m: the video layer reads r_edr to tell "on where the display can"
/// (1) from "ask anyway" (2), which is a distinction only it can act on, and
/// reads r_viewfbo and r_gamma_analytic so r_edr_report can name WHICH clause of
/// the ask is failing -- five ways of answering "nothing happened" that a player
/// cannot tell apart from the picture.
extern cvar_t r_edr;
extern cvar_t r_gamma_analytic;
...
// passes, never inside a draw batch; Sample returns 0 when there is no grid and
// otherwise (amb + 0.25 * dif) at lightmapintensity 1 -- the particle convention.
qbool R_Volumetric_ResolveIrradiance(void);
qbool R_Volumetric_SampleIrradiance(const vec3_t p, vec3_t out);
// Resolve the heat shimmer's baked world field during map load -- called from
// CL_UpdateScreen before the Metal backend opens the frame -- so the two
// backends never bake it at different sites (WHEN it bakes shifts the whole
// scene for the boot; see the definition).
void R_Volumetric_ResolveField(void);

// The MetalFX-temporal arc's REACTIVE STAMP (2026-08-19): a transparent batch
// that the scene just drew -- particles today; the explosion shell or sprites
// could follow, which is why the interface is generic -- appends its own
// vertex arrays here, and the reactive-mask pass replays them into the mask so
// the temporal scaler distrusts its history exactly where the batch landed.
// Particles write no depth and carry no motion vector, so nothing else can
// tell the scaler they are there: 78% of what moves behind a rocket is its
// smoke trail. Pure memcpy into a persistent stash; inert unless the mask
// pass armed it this frame (R_ReactiveStamp_Active), so a call from a draw
// path costs one branch. vertex3f is world space; color4f is the batch's own
// premultiplied colour (coverage for alpha blends, intensity for additive --
// the stamp weights by whichever is larger); texcoord2f indexes `texture`.
...
/// The MetalFX-TEMPORAL arc's single predicate: is the temporal upscaling path
/// selected this frame? Forces the offscreen scene path and a SAMPLEABLE depth
...
extern rtexture_t *r_texture_white3d;
...
#ifdef USE_RT_METAL
struct rt_reproj_s;
/// METAL.md Phase 5: select the RT composite's shader, bind the sidecar's term
/// texture by handle and upload the reprojection block. Metal path only.
void R_SetupShader_RTComposite(unsigned int termhandle, const struct rt_reproj_s *rp, rtexture_t *depthtexture, int termw, int termh);
#endif
void R_SetupShader_Generic_NoTexture(qbool usegamma, qbool notrippy);
void R_SoftParticles_Batch(float fadedist);	// BEAUTY A4: per particle batch, 0 = no fade
extern cvar_t cl_particles_soft;
void R_PartRefract_Batch(float strength, float cs, float ct, float invhalf);	// BEAUTY A5: per refract batch, 0 = none
extern cvar_t cl_particles_refract;
...
typedef struct r_framebufferstate_s
{
	...
	// WATERSURFACE (r_watersurface): a copy of the composited frame -- RT term,
	// glow, murk -- taken inside R_RenderScene after the murk and before the
	// transparent pass, so a blended liquid can refract what lies beneath it.
	// Screen-sized and of textype, like the ghost texture. waterseen is set where
	// a blended liquid batch is classified for this view, so a frame with no water
	// in sight takes no copy; waterscreen_valid says the copy was taken this frame.
	rtexture_t *waterscreen;
	qbool waterscreen_valid;
	qbool waterseen;
	qbool refractseen;   // BEAUTY A5: a PBLEND_REFRACT particle was queued for this view, so the copy is worth taking
	qbool usedepthtextures; // use depth texture instead of depth renderbuffer (faster if you need to read it later anyway)

	// VOLUMETRICS (r_volumetric): the camera's scene depth, published as reusable
	// scene state once the 3D view has been rendered, so any later screen-space pass
	// can reconstruct view- and world-space position per pixel. Filled in
	// R_RenderView for the MAIN view only; scenedepthvalid is 0 on every other view
	// and whenever r_volumetric is off, so consumers must check it first.
	//
	// Reconstruction (matches the MODE_DEFERREDLIGHTSOURCE convention):
	//   z_view = scenedepth_screentodepth.y / (depth_sample + scenedepth_screentodepth.x)
	//   xy_view = ndc * vec2(scenedepth_frustum_x, scenedepth_frustum_y) * -z_view
	//   world   = scenedepth_viewtoworld * vec4(view, 1)
	// NOTE the depth texture is an ATTACHMENT of rt_screen, so a pass may only sample
	// it while rendering into a DIFFERENT target — sampling a texture attached to the
	// bound framebuffer is a feedback loop and undefined in GL.
	rtexture_t *scenedepthtexture;      // rt_screen's depth attachment, sampleable
	matrix4x4_t scenedepth_viewtoworld; // inverse of the GL view matrix
	float scenedepth_screentodepth[2];  // window depth -> view-space z
	float scenedepth_frustum_x;         // tan(hfov/2)
	float scenedepth_frustum_y;         // tan(vfov/2)
	float scenedepth_farclip;           // clamp for sky / no-hit pixels
	int scenedepthvalid;                // nonzero when the above describe this frame
	qbool volumetricactive;             // the murk pass will run this frame, so opaque surfaces must skip the engine's own per-fragment fog
	qbool rtcompositepending;           // macOS Metal RT: composite is due inside this scene render, before the murk
	qbool rtglowpending;                // macOS Metal RT: wall lighting will multiply this scene, so opaque passes withhold emission for the glow re-add pass (rt_metal_glowpass)

	// The MetalFX-TEMPORAL arc. Temporal upscaling needs three things the engine
	// has never kept: a sub-pixel jitter applied to the raster projection, the
	// PREVIOUS frame's camera to reproject against, and a per-frame reset flag
	// for the discontinuities a history cannot survive. All of it is main-view
	// only and Metal only; taavalid is 0 on every other view and whenever the
	// temporal path is not selected, so consumers check it first.
	//
	// The previous camera is stored as the viewport's own VIEW matrix plus the
	// frustum scalars, NOT as a view-projection: the motion-vector pass
	// reconstructs world position exactly as the murk does (ndc * FrustumScale *
	// dist through ViewToWorld) and then runs that reconstruction BACKWARDS
	// through the previous frame's view matrix and frustum. Every convention it
	// uses is therefore one the murk already proves, and no clip-space
	// handedness question arises on either side.
	qbool taawanted;                    // the temporal path is selected this frame (drives the jitter and the depth-as-texture force)
	qbool taavalid;                     // taa_prevview / taa_prevfrustum describe the PREVIOUS frame
	qbool taareset;                     // discard the accumulated history on this frame's encode
	matrix4x4_t taa_prevview;           // previous frame's viewport.viewmatrix (world -> previous view)
	float taa_prevfrustum[2];           // previous frame's frustum_x / frustum_y
	float taa_prevorigin[3];            // previous frame's eye, for the teleport test
	matrix4x4_t taa_prevviewmodel;      // previous frame's viewmodelmatrix_withbob (model -> world)
	qbool taa_prevviewmodelvalid;       // taa_prevviewmodel describes the PREVIOUS frame
	float taa_jitter[2];                // this frame's sub-pixel offset, in render pixels
	unsigned int taa_frame;             // advances once per main-view render; indexes the Halton sequence
	// T2b: the previous-frame model matrix of every network entity, keyed on
	// entitynumber (stable for network entities; 0 for temp entities, which take
	// the camera-only fill). Rigid-body only by design -- limb motion inside a
	// walk cycle is not captured. Grown on demand to cl.max_entities.
	struct taa_prevent_s { matrix4x4_t matrix; unsigned int stamp; } *taa_prevent;
	int taa_prevent_max;
