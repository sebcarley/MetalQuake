...
===============
*/
cvar_t m5_sharedconfig = {CF_CLIENT | CF_ARCHIVE, "m5_sharedconfig", "1", "save one config for every game, in the m5 gamedir, instead of a separate one per gamedir. Without it a mission pack keeps its own complete copy of your config, so a keybind or mouse setting changed in Quake does not follow you into Armagon. Per-pack look differences belong in m5pack_<gamedir>.cfg"};

/// True if the named gamedir is currently mounted.
qbool M5_GameDirMounted(const char *dir)
{
	int i;
	for (i = 0; i < fs_numgamedirs; i++)
		if (!strcasecmp(fs_gamedirs[i], dir))
			return true;
	return false;
}
...
// config.cfg cvars
	...
	{
		// M5: one config for every game, not one per gamedir.
		//
		// FS_OpenRealFile writes into the PRIMARY gamedir, which is whichever
		// is last -- so with a mission pack mounted the config lands in the
		// pack's own folder, and every gamedir slowly accumulates a divergent
		// copy of the entire config. A keybind or mouse setting changed in
		// Quake then does not follow you into Armagon, and it is easy to end
		// up tuning the same setting to three different values without
		// noticing. Saving into m5 instead makes the search path do the rest:
		// "exec config.cfg" walks the gamedirs and falls through to m5's once
		// no pack has one of its own.
		//
		// Per-pack differences belong in m5pack_<gamedir>.cfg, which is exec'd
		// after this one, rather than in five copies of everything.
		if (m5_sharedconfig.integer && M5_GameDirMounted("m5") && !strcmp(file, CONFIGFILENAME))
			f = FS_OpenRealFileInGameDir("m5", file, "wb", false);
		else
...
static void Host_AddConfigText(cmd_state_t *cmd)
{
	...
	// M5 ball lightning (BALLLIGHTNING.md, Seb: "bake it into the game"): key 9
	// selects the ninth weapon -- APPENDED so it runs after the quake.rc chain,
	// because id1's default.cfg opens with unbindall and would wipe a bind set
	...
	// across the games. Anything a pack needs differently from stock Quake
	// (light level, red glow, fog) lives here and nowhere else, so switching
	// game changes only what the pack asked to change.
	if (fs_numgamedirs > 0 && strcasecmp(fs_gamedirs[fs_numgamedirs - 1], "m5"))
	{
		char packcfg[MAX_QPATH], line[MAX_QPATH + 8];
		dpsnprintf(packcfg, sizeof(packcfg), "m5pack_%s.cfg", fs_gamedirs[fs_numgamedirs - 1]);
		if (FS_FileExists(packcfg))
		{
			dpsnprintf(line, sizeof(line), "exec %s\n", packcfg);
			Cbuf_AddText(cmd, line);
		}
	}
}
...
static void Host_InitLocal (void)
{
	...
	Cvar_RegisterVariable (&m5_sharedconfig);
...
// COMMANDLINEOPTION: Client: -capturedemo <demoname> captures a playdemo and quits
	...
	// METAL.md Phase 1: open the command digest here, after the filesystem is
	// up (it writes through FS_OpenRealFile) and before any frame is rendered.
	// Compiles to nothing without -DDP_CMDTRACE.
	DPCMD_Init();
...
void Host_Shutdown(void)
{
	...
	// close the command digest before the filesystem goes away
	DPCMD_Shutdown();
