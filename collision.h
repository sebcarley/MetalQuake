...
// read-only diagnostics: fire a deterministic, seeded set of point/box/surface
// traces against a model's collision and write full-precision results to a file,
// so refactors to the ray-cast core can be proven bit-identical (gameplay safety)
// and benchmarked (traces/sec). Exposed via the "collision_dumptraces" command.
void Collision_TraceParityTest(struct model_s *model, int numtraces, unsigned int seed, const char *outname);

// export the world render mesh (vertices + triangle indices) to a text file, so
// an external tool can build a ray-tracing acceleration structure from the same
// geometry the engine traces against.
