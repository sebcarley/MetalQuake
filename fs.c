			...
			if (split)
			{
				...
				else
				{
					...
					while (split > fs_basedir && *split != '/')
						split--;
					*split = 0;
					// M5: PREFER a "packs" folder beside the .app. That is the
					// shipped layout -- <somewhere>/QuakeM5.app next to
					// <somewhere>/packs/{id1,m5,hipnotic,rogue,dopa,mg1,ad} --
					// so the app can sit in /Applications or a games folder
					// with the game data in ONE tidy sibling rather than seven
					// loose gamedirs spilled around it. Checked before the bare
					// sibling layout below, which stays supported: an existing
					// install with id1 directly beside the .app keeps working,
					// and this branch simply never fires for it.
					if (stat(va(vabuf, sizeof(vabuf), "%s/packs/%s", fs_basedir, gamedirname1), &statresult) == 0)
						dp_strlcat(fs_basedir, "/packs", sizeof(fs_basedir));
					// if the gamedir isn't next to the .app either (an
					// Xcode DerivedData build, say), fall back to the working
					// directory — the QuakeM5 scheme points it at the repo
					else if (stat(va(vabuf, sizeof(vabuf), "%s/%s", fs_basedir, gamedirname1), &statresult) != 0)
					{
						char cwdbuf[1024];
						// the repo itself may carry the packs layout too, so try
						// the working directory both ways round before giving up
						if (getcwd(cwdbuf, sizeof(cwdbuf)))
						{
							if (stat(va(vabuf, sizeof(vabuf), "%s/%s", cwdbuf, gamedirname1), &statresult) == 0)
								dp_strlcpy(fs_basedir, cwdbuf, sizeof(fs_basedir));
							else if (stat(va(vabuf, sizeof(vabuf), "%s/packs/%s", cwdbuf, gamedirname1), &statresult) == 0)
								dpsnprintf(fs_basedir, sizeof(fs_basedir), "%s/packs", cwdbuf);
						}
					}
				}
			}
...
/*
====================
FS_OpenRealFileInGameDir

As FS_OpenRealFile, but writes into a NAMED gamedir instead of the primary
one. FS_OpenRealFile prefixes fs_gamedir, which is whichever gamedir happens
to be last -- so with a mission pack mounted the config lands in the pack's
folder and every gamedir accumulates its own divergent copy of everything.
M5 uses this to keep one shared config; see Host_SaveConfig.

Writes to the userdir when there is one, matching FS_AddGameHierarchy's own
basedir-then-userdir order, so it lands where the per-gamedir configs already
live rather than in the game data.
====================
*/
qfile_t* FS_OpenRealFileInGameDir (const char* gamedir, const char* filepath, const char* mode, qbool quiet)
{
	char real_path [MAX_OSPATH];

	if (FS_CheckNastyPath(gamedir, true) || FS_CheckNastyPath(filepath, false))
	{
		Con_Printf("FS_OpenRealFileInGameDir(\"%s\", \"%s\"): nasty path rejected\n", gamedir, filepath);
		return NULL;
	}
	dpsnprintf (real_path, sizeof (real_path), "%s%s/%s", *fs_userdir ? fs_userdir : fs_basedir, gamedir, filepath);
