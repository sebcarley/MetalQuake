...
static void CL_ClientMovement_Physics_Noclip(cl_clientmovement_state_t *s)
{
	// M5 noclip flight (m5_noclipfly): the prediction mirror of SV_AirMove's
	// noclipfly arm -- full-view-angle basis, additive upmove, the maxspeed
	// clamp on the 3D magnitude, velocity assigned directly (no friction, no
	// acceleration ramp), origin integrated with no trace and no gravity.
	// LOCKSTEP: SV_AirMove (sv_user.c) -- change one, change both. Any drift
	// between the two is a per-frame prediction warp the player can feel.
	vec3_t forward, right, up, wishvel;
	vec_t wishspeed;

	AngleVectors(s->cmd.viewangles, forward, right, up);
	VectorMAM(s->cmd.forwardmove, forward, s->cmd.sidemove, right, wishvel);
	wishvel[2] += s->cmd.upmove;

	wishspeed = VectorLength(wishvel);
	if (wishspeed > cl.movevars_maxspeed)
		VectorScale(wishvel, cl.movevars_maxspeed / wishspeed, wishvel);

	VectorCopy(wishvel, s->velocity);
	s->onground = false;
	VectorMA(s->origin, s->cmd.frametime, s->velocity, s->origin);
}
...
static void CL_ClientMovement_PlayerMove(cl_clientmovement_state_t *s)
{
	...
	CL_ClientMovement_UpdateStatus(s);
	// M5 noclip flight: both bits required -- the server has the behaviour on
	// AND this client is actually in MOVETYPE_NOCLIP this frame. With either
	// missing, prediction keeps its old (swim/walk) behaviour exactly.
	if ((cl.moveflags & MOVEFLAG_NOCLIPFLY) && (cl.moveflags & MOVEFLAG_NOCLIPPING))
		CL_ClientMovement_Physics_Noclip(s);
	else if (s->waterlevel >= WATERLEVEL_SWIMMING)
