...
// SEPTEMBER S7: the ambient dust emitter, called once per client frame from
// CSQC_RelinkAllEntities beside M5_Torch_Relink (the view matrix is valid there)
...
typedef enum
{
	...
	PBLEND_ADD = 1,
	PBLEND_INVMOD = 2,
	PBLEND_REFRACT = 3,   // BEAUTY A5: the particle displaces the frame behind it (the shockwave)
	PBLEND_INVALID = -1
}
pblend_t;
...
typedef enum ptype_e
{
	...
	pt_dust,      // M5 ambient dust motes (SEPTEMBER S7, m5_dust) -- lit, recycled around the eye
	pt_total
}
ptype_t;
...
//	float           delayedcollisions; ///< time that p->bounce becomes active
	...
	unsigned char   viewmodel;  ///< BEAUTY A3: drawn with the depth test off, over the view weapon, as a weapon flash is drawn everywhere -- the local muzzle flash
}
particle_t;
