...
#ifdef CONFIG_MENU
	// METAL.md Phase 0: the Metal path has no renderer, so loading a map would
	// walk the client into the GL texture uploader with no GL context behind it.
	// Refuse with one honest line instead. REMOVE at Phase 3.
#endif
...
// read-only diagnostics: fire a deterministic, seeded set of collision traces
// against the loaded world and dump full-precision results. Diffing the dump
// before vs after a change to the ray-cast core proves it stays bit-identical
// (gameplay safety); the reported traces/sec benchmarks that same core. Runs on
// a dedicated server (headless), so it needs no video context.
// Usage: collision_dumptraces [count] [seed] [filename]
static void SV_CollisionDumpTraces_f(cmd_state_t *cmd)
{
	int numtraces = 20000;
	unsigned int seed = 1;
	const char *filename = "tracedump.txt";

	if (!sv.active || !sv.worldmodel)
	{
		Con_Printf("collision_dumptraces: no map loaded (run 'map <name>' first)\n");
		return;
	}
	if (Cmd_Argc(cmd) >= 2)
		numtraces = atoi(Cmd_Argv(cmd, 1));
	if (Cmd_Argc(cmd) >= 3)
		seed = (unsigned int)strtoul(Cmd_Argv(cmd, 2), NULL, 0);
	if (Cmd_Argc(cmd) >= 4)
		filename = Cmd_Argv(cmd, 3);

	Collision_TraceParityTest(sv.worldmodel, numtraces, seed, filename);
}

// read-only diagnostics: export the world render triangles for the Metal RT
// parity probe (build an acceleration structure from the same geometry).
static void SV_CollisionExportWorldTris_f(cmd_state_t *cmd)
{
	const char *filename = "worldtris.txt";
	if (!sv.active || !sv.worldmodel)
	{
		Con_Printf("collision_exportworldtris: no map loaded (run 'map <name>' first)\n");
		return;
	}
	if (Cmd_Argc(cmd) >= 2)
		filename = Cmd_Argv(cmd, 1);
	Collision_ExportWorldTriangles(sv.worldmodel, filename);
}
...
void SV_InitOperatorCommands(void)
{
	...
	Cmd_AddCommand(CF_SERVER, "collision_dumptraces", SV_CollisionDumpTraces_f, "fire a deterministic set of collision traces against the world and dump full-precision results to a file, for verifying ray-cast core changes stay bit-identical and benchmarking traces/sec: collision_dumptraces [count] [seed] [filename]");
	Cmd_AddCommand(CF_SERVER, "collision_exportworldtris", SV_CollisionExportWorldTris_f, "export world render triangles (verts + indices) to a file for the Metal ray-tracing parity probe: collision_exportworldtris [filename]");
