...
// M5: the enhanced thunderbolt (r_lightningbeam_m5). The stock beam is one
// straight quad; this is a midpoint-displaced polyline with forked branches and
// braided filaments, re-rolled on a fixed cadence so the shape crackles at the
// same rate whatever the framerate. Built once per beam per frame in
// CL_RelinkBeams and consumed by both the light push and the geometry emit, so
// the lights sit on the path the player can actually see.
// The base bolt is 96 points / 12 lines (channel 33 + 3 filaments x 9 + 4 forks
// x 9; core 3 passes + inner + sheath + 3 + 4). Frizzle adds a branching tree
// plus micro-streamers at every free tip, twice over - once for this roll and
// once for the previous roll's fading ghost. Every frizzle loop is guarded on
// these caps exactly as the fork and filament loops are, so an absurd cvar drops
// late frizzle rather than corrupting the arrays.
// Sized for the worst case at the SHIPPED frizzle depth: 5 free tips (channel +
// 4 forks), each fraying into 2 + 4 branches of 5 nodes and up to 3 three-node
// streamers, twice over = 486 points / 102 lines. r_lightningbeam_m5_frizzle_gens
// 3 can ask for more than this; the guards drop the ghost's last branches, which
// is why the ghost is generated second.
#define M5BOLT_MAXPOINTS	560		///< base 96 + frizzle tree and streamers, live set and ghost
#define M5BOLT_MAXLINES		128		///< base 12 + frizzle tree and streamers, live set and ghost

typedef struct m5boltline_s
{
	int		first, count;			///< range into m5boltpath_t.points
	float	width, widthtip;		///< ribbon half-width in world units, root and tip
	float	alpha, alphatip;		///< vertex alpha; the material is additive, so this is the dimmer
	float	toffset, trepeat;		///< S phase offset and repeat multiplier, so lines never overlay identically
	float	roll;					///< radians about the per-node tangent. 0 is the plain camera-facing card; the core's extra passes are rolled off it to fake a tube
	int		whiten;					///< 1 = lift the hue toward white (the hot core)
	float	taperstart;				///< arc fraction past which width and alpha ramp to nothing, so a free tip dissolves instead of ending in a blunt quad. 0 = no terminal taper and the width/alpha lerp is byte-exactly the old one
}
m5boltline_t;

typedef struct m5boltpath_s
{
	vec3_t			points[M5BOLT_MAXPOINTS];
	m5boltline_t	lines[M5BOLT_MAXLINES];
	int				numpoints, numlines;
	int				numfrizzlines;				///< how many of the LAST lines are frizzle (this roll's fray plus the previous roll's ghost). 0 with r_lightningbeam_m5_frizzle off
	int				channelfirst, channelcount;	///< the main channel, which the light walk samples
	float			length;						///< straight start->end distance
	float			envelope;					///< flicker envelope, clamped to [M5_FLICKER_MIN, 1]
	float			hit;						///< 1 while doing damage, decaying to 0 after - see r_lightningbeam_m5_hitfade
	vec3_t			hue;						///< max-channel-normalised colour the player actually sees
}
m5boltpath_t;
...
typedef struct rtlight_s
{
	...
	/// LIGHTFLAG_* flags
	int flags;
	/// M5: how loudly this light counts in the VOLUMETRIC FOG - both what it
	/// scatters and its say in the fog kernel's dominant-light vote. Surfaces
	/// ignore it, which is what lets the bolt's fog contribution be dialled
	/// independently of the light it casts on walls.
	///
	/// R_RTLight_Update memsets the whole struct, so this arrives as 0 for every
	/// caller that does not care. 0 therefore means "unset" and is read as 1.0 by
	/// RT_GatherDynamicLights - which is why nothing else in the tree needed
	/// touching when this was added.
	vec_t m5fogweight;
	/// M5 (F6, the handlamp): a SPOT CONE for this light. m5spotdir is the unit
	/// direction it throws; ALL ZERO means omni and is the no-op sentinel, which
	/// every kernel honours by multiplying its contribution by exactly 1.0f.
	/// m5spotcos is the cosine of the outer half-angle, read only when the
	/// direction is non-zero.
	///
	/// Same free-by-memset contract as m5fogweight above: R_RTLight_Update clears
	/// the whole struct, so all eight existing producers keep pushing omni lights
	/// with no edit. Only the handlamp sets these.
	vec3_t m5spotdir;
	vec_t m5spotcos;
	/// Cosine of the INNER half-angle -- where the cone reaches full brightness.
	/// The shoulder runs from m5spotcos to this. Same memset contract: <= m5spotcos
	/// (which a zero is) means "unset", and the kernels fall back to the historic
	/// fixed 35%-of-the-way-to-the-axis shoulder, so nothing else needed touching.
	vec_t m5spotcosinner;
	/// FILAMENT (2026-09-06, m5_torch_filament): 0-1, how strongly the beam shows
	/// its bulb -- a hot core about a third of the beam across with a faint dark
	/// ring outside it, the way a cheap reflector images its filament. Same memset
	/// contract: 0 is "none" and rt_cone multiplies by exactly 1.0 for it.
	vec_t m5spotfilament;
...
{
	// SEPTEMBER2 D (2026-09-06): the map's sun, from worldspawn's ericw keys (_sunlight, _sunlight_color, _sunlight_mangle / _sun_mangle, _sunlight_penumbra); cleared per map by CL_ParseEntityLump
	float m5sun_light;
	float m5sun_color[3];
	float m5sun_mangle[3];
	float m5sun_penumbra;
	int m5sun_havemangle;
...
//[515]: added for csqc purposes
	...
	// used by the idle view sway (view.c). Zeroed with the rest of cl on
	// connect, which is exactly the reset we want.
	double idlesway_idletime;	///< seconds of no movement, no buttons and no mouse
	float idlesway_ramp;		///< 0-1, eased towards 1 once idletime passes the delay
	vec3_t idlesway_prevangles;	///< last frame's view angles, to spot the mouse moving
...
// M5 fun mods (cl_main.c / cl_particles.c)
extern cvar_t m5_shotgun;
extern cvar_t m5_shotgun_casing;
extern cvar_t m5_venom;			// Scrag spit reads as acid; the trail half lives in cl_particles.c
extern cvar_t m5_venom_trail;
extern cvar_t m5_balllightning_violence;	// the ball's boil ramp, read by r_lightning.c's knot producer
qbool M5_ShotgunBoosted(const entity_t *ent);
void M5_ShotgunShellEject(entity_t *ent);
extern cvar_t m5_muzzleflash;		// BEAUTY A3: the drawn muzzle flash (cl_particles.c)
void M5_MuzzleFlash_Arm(entity_t *ent);		// rising edge of EF_MUZZLEFLASH (cl_parse.c)
void M5_MuzzleFlash_Update(void);
void CL_ParticleFont_Update(void);	// BEAUTY A2: the atlas re-generates when cl_particles_texsize moves (cl_main.c)
void CL_TorchEmbers_Update(void);	// BEAUTY B4: embers off the torches, the relink window (cl_main.c)
void CL_SkyLightning_Update(void);	// BEAUTY C1: the storm's schedule, sun, fog light and thunder, the relink window (cl_screen.c)
qbool CL_IsLightCoreModelName(const char *name);	// BEAUTY B4: the torch/brazier/candle model names (cl_particles.c, so the dedicated server links)		// the relink window, beside CL_Dust_Update (cl_main.c)
...
// M5 enhanced thunderbolt: the master gate, so CL_RelinkBeams can choose the
// renderer. The rest of the r_lightningbeam_m5_* family is private to
// r_lightning.c apart from the menu's own extern block.
extern cvar_t r_lightningbeam_m5;
extern cvar_t r_lightningbeam_m5_sdf;
extern cvar_t r_lightningbeam_m5_sdf_falloff;
extern cvar_t r_lightningbeam_m5_fizz;
extern cvar_t r_lightningbeam_m5_fizz_scale;
extern cvar_t r_lightningbeam_m5_fizz_speed;
int CL_Beam_M5_SDFFrame(void);
// M5 enhanced thunderbolt. BuildPath returns false when the feature cannot run
// (degenerate beam, scene mesh nearly full), in which case the caller must fall
// back to the stock path.
qbool CL_Beam_M5_BuildPath(const beam_t *b, const vec3_t start, const vec3_t end, m5boltpath_t *out);
/// F1's capsule-SDF path. Active decides where CL_Beam_M5_AddPolygons sends the
/// bolt; the renderer owns the latch and reads the stream back in the same frame.
qbool CL_Beam_M5_SDFActive(void);
void CL_Beam_M5_SDFSetLatch(qbool ok);
const void *CL_Beam_M5_SDFSegments(int *count);
int CL_Beam_M5_SDFSegmentBytes(void);
void CL_Beam_M5_AddPolygons(const m5boltpath_t *p);
void CL_Beam_M5_AddLights(const m5boltpath_t *p);
// M5 ball lightning (r_lightning.c, BALLLIGHTNING.md slice 3): the plasma knot
// drawn at an EF_M5BALL entity, and the colour/flicker of the light cl_main.c
// hangs on it
...
float CL_Beam_M5_BallPressure(void);		// 0..1 screen pressure from the nearest ball this frame, times its cvar
