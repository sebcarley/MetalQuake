...
// hack to not let you back into teleporter
	...
	if (m5_noclipfly.integer && (int)PRVM_serveredictfloat(host_client->edict, movetype) == MOVETYPE_NOCLIP)
	{
		// M5 noclip flight (m5_noclipfly): fly where you look. Build the wish
		// basis from the FULL view angles -- v_angle carries the real pitch,
		// where the yaw-only basis above deliberately discards it (that is the
		// LadyHavoc walking-speed fix, correct for walking, wrong for flight).
		// Locals on purpose: forward/right/up are file-scope globals shared
		// with SV_WaterMove and the accelerate helpers, and this arm must not
		// change what they hold. The sv_maxspeed clamp below normalises the
		// 3D magnitude, so looking straight up flies at full speed rather
		// than the projected fraction. The additive upmove keeps the swim
		// keys working, in the shared non-WALK branch below.
		// LOCKSTEP: CL_ClientMovement_Physics_Noclip (cl_input.c) mirrors
		// this arithmetic exactly -- change one, change both.
		vec3_t nangles, nforward, nright, nup;
		VectorCopy(PRVM_serveredictvector(host_client->edict, v_angle), nangles);
		AngleVectors(nangles, nforward, nright, nup);
		for (i=0 ; i<3 ; i++)
			wishvel[i] = nforward[i]*fmove + nright[i]*smove;
	}
	else
	{
		for (i=0 ; i<3 ; i++)
