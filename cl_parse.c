...
void CL_ParseEntityLump(char *entdata)
{
	...
	// SEPTEMBER2 D: no sun until the map declares one
	cl.m5sun_light = 0; cl.m5sun_color[0] = cl.m5sun_color[1] = cl.m5sun_color[2] = 1.0f; VectorClear(cl.m5sun_mangle); cl.m5sun_penumbra = 0; cl.m5sun_havemangle = 0;
...
#define sscanf sscanf_s
		...
		// SEPTEMBER2 D: ericw's sun keys (the leading underscore is already stripped above).
		// _sun_mangle is the older spelling AD's start map uses; _sunlight_mangle the newer.
		else if (!strcmp("sunlight", key))
			cl.m5sun_light = atof(value);
		else if (!strcmp("sunlight_color", key))
		{
			if (sscanf(value, "%f %f %f", &cl.m5sun_color[0], &cl.m5sun_color[1], &cl.m5sun_color[2]) == 3)
				if (cl.m5sun_color[0] > 1.0f || cl.m5sun_color[1] > 1.0f || cl.m5sun_color[2] > 1.0f)   // ericw's own heuristic: any component above 1 means 0-255 (the m5_lightcolor255 fact)
					VectorScale(cl.m5sun_color, 1.0f / 255.0f, cl.m5sun_color);
		}
		else if (!strcmp("sunlight_mangle", key) || !strcmp("sun_mangle", key))
		{
			if (sscanf(value, "%f %f %f", &cl.m5sun_mangle[0], &cl.m5sun_mangle[1], &cl.m5sun_mangle[2]) >= 2)
				cl.m5sun_havemangle = 1;
		}
		else if (!strcmp("sunlight_penumbra", key))
			cl.m5sun_penumbra = atof(value);
...
void CL_MoveLerpEntityStates(entity_t *ent)
{
	...
	{
		// rising edge = a new shot; M5 Doom shotgun ejects a shell casing,
		// and BEAUTY A3 draws the flash Quake never had
		if (ent->persistent.muzzleflash <= 0)
		{
			M5_ShotgunShellEject(ent);
			M5_MuzzleFlash_Arm(ent);
		}
