...
// M5: noclip flies where you look (m5_noclipfly). Two bits because prediction
// needs two facts the client otherwise cannot know: that the server has the
// behaviour enabled, and that THIS client is in MOVETYPE_NOCLIP right now
// (cl_clientmovement_state_t carries no movetype and no stat ever sent one).
#define MOVEFLAG_NOCLIPFLY 0x00000008
#define MOVEFLAG_NOCLIPPING 0x00000010
