...
cvar_t m5_noclipfly = {CF_SERVER | CF_ARCHIVE, "m5_noclipfly", "1", "noclip flies where you look: forward/back move along the full view direction, pitch included, at full speed (the Quake Enhanced behaviour). Swim keys still add vertically. 0 restores the classic yaw-only noclip"};
...
cvar_t m5_hasplasma = {CF_SERVER | CF_READONLY, "m5_hasplasma", "0", "set by the engine at every map start: 1 when progs/v_plasma.mdl can be found, 0 when it cannot. The ball lightning's view weapon is Dissolution of Eternity's plasma gun, which is the mission pack's own file and cannot ship in a public build; the QuakeC reads this and carries the thunderbolt's model instead when it is absent, rather than precaching a file that is not there"};
...
void SV_SpawnServer (const char *map)
{
	...
	// before the progs run worldspawn, which is where the precache list is built
	Cvar_SetValueQuick(&m5_hasplasma, FS_FileExists("progs/v_plasma.mdl") ? 1 : 0);
