...
// last completed frame's counters, for r_speeds_dump (METAL.md Phase 4a)
static int r_stat_lastframe[r_stat_count];
...
, r_speeds_timestring);
	...
	// snapshot for r_speeds_dump before the zero -- a console command runs
	// between frames and would otherwise always read zeros (METAL.md Phase 4a)
	memcpy(r_stat_lastframe, r_refdef.stats, sizeof(r_stat_lastframe));
	memset(&r_refdef.stats, 0, sizeof(r_refdef.stats));
}

// METAL.md Phase 4a: the r_stat_* counters exist only as an on-screen overlay
// (r_speeds), which a headless parity run cannot read. This dumps them to the
// console once, so a GL run and a Metal run can assert IDENTICAL draw and
// vertex counts -- the free cross-check METAL.md's parity harness names: the
// counters are incremented in R_Mesh_Draw BEFORE the renderpath switch, so any
// batching difference shows up here even when the pixels happen to agree.
//
// Values are the LAST COMPLETED frame's, snapshotted in R_TimeReport_EndFrame
// just before it zeroes the live block (which a console command would otherwise
// always read as zero).
void R_Stats_Dump_f(cmd_state_t *cmd)
{
	int i;
	qbool all = (Cmd_Argc(cmd) >= 2 && !strcmp(Cmd_Argv(cmd, 1), "all"));
	Con_Printf("r_speeds_dump: last completed frame\n");
	for (i = 0; i < r_stat_count; i++)
		if (all || r_stat_lastframe[i])
			Con_Printf("  %-40s %d\n", r_stat_name[i], r_stat_lastframe[i]);
}
