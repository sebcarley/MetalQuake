...
#include "quakedef.h"
#include "image.h"
#include "metal_textures.h"   // METAL.md Phase 3: the Metal texture objects
#include "jpeg.h"
#include "image_png.h"
...
void R_FreeTexture(rtexture_t *rt)
{
	...
	case RENDERPATH_METAL:
		Metal_Texture_Destroy(glt->texnum);
		glt->texnum = 0;
		break;
...
// METAL.md Phase 3 acceptance test. Nothing renders on the Metal path yet, so
// "the log had no errors" proves nothing -- an upload that silently did not
// happen is indistinguishable from one that worked. This drives a known pattern
// through the REAL R_LoadTexture2D path and reads it back off the GPU.
// Console-only, macOS-only, and it costs nothing when not called.
static void R_Metal_TextureProbe3D(void);

static void R_Metal_TextureProbe_f(cmd_state_t *cmd)
{
	static const int W = 16, H = 16;
	unsigned char src[16 * 16 * 4], back[16 * 16 * 4];
	rtexturepool_t *pool;
	rtexture_t *rt;
	int i, bad = 0;
	(void)cmd;
	...
	{
		Con_Printf("r_metal_textureprobe: only meaningful on the Metal path (vid_renderer metal)\n");
		return;
	}
	// a pattern with no symmetry, so a transposed or channel-swapped readback
	// cannot accidentally compare equal
	for (i = 0; i < W * H; i++)
	{
		src[i * 4 + 0] = (unsigned char)(i * 7);
		src[i * 4 + 1] = (unsigned char)(i * 13 + 5);
		src[i * 4 + 2] = (unsigned char)(255 - i * 3);
		src[i * 4 + 3] = (unsigned char)(i | 0x40);
	}
	pool = R_AllocTexturePool();
	rt = R_LoadTexture2D(pool, "metalprobe", W, H, src, TEXTYPE_BGRA, TEXF_CLAMP | TEXF_FORCENEAREST, -1, NULL);
	if (!rt)
	{
		Con_Printf(CON_ERROR "r_metal_textureprobe: R_LoadTexture2D returned NULL\n");
		R_FreeTexturePool(&pool);
		return;
	}
	// force the deferred upload exactly the way a bind would
	R_RealGetTexture(rt);
	memset(back, 0, sizeof(back));
	if (!Metal_Texture_Readback(rt->texnum, back))
	{
		Con_Printf(CON_ERROR "r_metal_textureprobe: readback failed (handle %d)\n", rt->texnum);
		R_FreeTexturePool(&pool);
		return;
	}
	for (i = 0; i < W * H * 4; i++)
		if (src[i] != back[i])
			bad++;
	Con_Printf("r_metal_textureprobe: handle %d, %d of %d bytes differ -- %s\n",
	           rt->texnum, bad, W * H * 4, bad ? "FAIL" : "PASS (upload and readback are byte-exact)");
	R_FreeTexturePool(&pool);
	R_Metal_TextureProbe3D();
}

// METAL.md Phase 6, slice 6-1. The 3D half, and it is deliberately TWO tests
// rather than one, because they fail on different things:
//
//   upload  -- the whole volume read back byte-exact. Catches a wrong
//              bytesPerImage, a collapsed z (only slice 0 written) and any
//              slice-stride slip. Cannot see how a shader ADDRESSES it.
//   sample  -- the renderer's own sampler, through a real compute dispatch.
//              Catches a flipped or transposed axis and a NEAREST-for-LINEAR
//              filter, neither of which disturbs a single byte of the upload.
//
// 6-2's murk arm rests entirely on the second one, which is why a readback
// alone was not accepted as the proof for this slice.
// The volume is deliberately NOT a cube, and the three extents are pairwise
// coprime-ish rather than tidy. An 8x8x8 probe cannot see a width/height
// transposition at all -- bytesPerRow and its transpose are the same number --
// and that blind spot is not hypothetical: a cube-shaped version of this probe
// was measured PASSING against a build whose bytesPerRow read `height * bpp`.
// At 8x4x6 the same break moves all 768 bytes and 255/255 of the sampling.
// 8x4x6 also makes the volume non-power-of-two in two axes, which is what the
// real baked field always is (its extents come from the map's bounds).
#define MP3_W 8
#define MP3_H 4
#define MP3_D 6
static void R_Metal_Probe3D_Texel(int x, int y, int z, unsigned char *p)
{
	// One channel per axis, so a transposed axis cannot compare equal: R is a
	// function of x ALONE, G of y alone, B of z alone. A is z-dependent too and
	// runs the opposite way, so a z-flip shows in two channels rather than one.
	// Every value stays inside 0..255 at 8x4x6 (max 215/78/149) -- no wrap, so
	// the CPU model is unambiguous. The z steps are EVEN (28, 30) so a midpoint
	// average is an exact integer and the linear-filter check needs no rounding
	// allowance.
	// A is never uniformly 255 (only at z=0), which keeps R_SetupTexture's
	// data-dependent TEXF_ALPHA auto-strip from quietly dropping the flag.
	p[0] = (unsigned char)(x * 30 + 5);     // R
	p[1] = (unsigned char)(y * 25 + 3);     // G
	p[2] = (unsigned char)(z * 28 + 9);     // B
	p[3] = (unsigned char)(255 - z * 30);   // A
}

static void R_Metal_TextureProbe3D(void)
{
	const int NX = MP3_W, NY = MP3_H, NZ = MP3_D, NT = MP3_W * MP3_H * MP3_D;
	unsigned char src[MP3_W * MP3_H * MP3_D * 4], back[MP3_W * MP3_H * MP3_D * 4];
	// Six texel centres, all with x, y and z mutually different and none equal
	// to another point's, so no axis permutation maps the set onto itself.
	static const int centres[6][3] = {{1,2,3},{5,0,5},{7,3,1},{0,1,4},{3,2,2},{6,0,0}};
	#define MP3_NPTS (6 + 5 + 2 + 2)
	float coords[MP3_NPTS * 3], got[MP3_NPTS * 4];
	rtexturepool_t *pool;
	rtexture_t *rt;
	int x, y, z, i, n = 0, bad = 0, maxdev = 0, worstsample = 0;
	...
	for (z = 0; z < NZ; z++)
		for (y = 0; y < NY; y++)
			for (x = 0; x < NX; x++)
				R_Metal_Probe3D_Texel(x, y, z, src + (((z * NY) + y) * NX + x) * 4);

	pool = R_AllocTexturePool();
	// TEXTYPE_RGBA with CLAMP + FORCELINEAR + ALPHA is EXACTLY the murk's baked
	// world field (R_Volumetric_GetField), so the format, the sampler and the
	// addressing under test are the ones Phase 6 will bind rather than a
	// plausible stand-in. The 2D half of this command already covers BGRA.
	rt = R_LoadTexture3D(pool, "metalprobe3d", NX, NY, NZ, src, TEXTYPE_RGBA,
	                     TEXF_CLAMP | TEXF_FORCELINEAR | TEXF_ALPHA, 0, NULL);
	if (!rt || !rt->texnum)
	{
		Con_Printf(CON_ERROR "r_metal_textureprobe: 3D texture creation FAILED (R_LoadTexture3D gave %s)\n",
		           rt ? "handle 0" : "NULL");
		R_FreeTexturePool(&pool);
		return;
	}
	...
	// --- upload: the whole volume, byte for byte -----------------------------
	memset(back, 0, sizeof(back));
	if (!Metal_Texture_Readback(rt->texnum, back))
	{
		Con_Printf(CON_ERROR "r_metal_textureprobe: 3D readback failed (handle %d)\n", rt->texnum);
		R_FreeTexturePool(&pool);
		return;
	}
	for (i = 0; i < NT * 4; i++)
		if (src[i] != back[i])
			bad++;
	Con_Printf("r_metal_textureprobe: 3D upload %dx%dx%d, %d of %d bytes differ -- %s\n",
	           NX, NY, NZ, bad, NT * 4,
	           bad ? "FAIL" : "PASS (every slice is byte-exact)");

	// --- sample: through the renderer's own MTLSamplerState ------------------
	for (i = 0; i < 6; i++)
	{
		coords[n * 3 + 0] = ((float)centres[i][0] + 0.5f) / (float)NX;
		coords[n * 3 + 1] = ((float)centres[i][1] + 0.5f) / (float)NY;
		coords[n * 3 + 2] = ((float)centres[i][2] + 0.5f) / (float)NZ;
		n++;
	}
	for (i = 0; i < 5; i++)   // exactly between z=i and z=i+1, x/y on centres
	{
		coords[n * 3 + 0] = 2.5f / (float)NX;
		coords[n * 3 + 1] = 1.5f / (float)NY;
		coords[n * 3 + 2] = ((float)i + 1.0f) / (float)NZ;
		n++;
	}
	for (i = 0; i < 2; i++)   // exactly between x=i+2 and x=i+3
	{
		coords[n * 3 + 0] = ((float)i + 3.0f) / (float)NX;
		coords[n * 3 + 1] = 1.5f / (float)NY;
		coords[n * 3 + 2] = 4.5f / (float)NZ;
		n++;
	}
	for (i = 0; i < 2; i++)   // exactly between y=i and y=i+1
	{
		coords[n * 3 + 0] = 4.5f / (float)NX;
		coords[n * 3 + 1] = ((float)i + 1.0f) / (float)NY;
		coords[n * 3 + 2] = 2.5f / (float)NZ;
		n++;
	}
	if (!Metal_Texture_Sample3D(rt->texnum, coords, got, n))
	{
		Con_Printf(CON_ERROR "r_metal_textureprobe: 3D sampling dispatch FAILED\n");
		R_FreeTexturePool(&pool);
		return;
	}
	for (i = 0; i < n; i++)
	{
		// The expected value is the CPU model evaluated at the same place: a
		// texel for a centre, the exact average of the two neighbours for a
		// midpoint. Both come out of ONE trilinear evaluation over the model, so
		// there is no second code path to get wrong. Note each axis is
		// denormalised by its OWN extent -- the whole point of a non-cubic
		// volume is that using the wrong one here would show up.
		float fx = coords[i * 3 + 0] * (float)NX - 0.5f;
		float fy = coords[i * 3 + 1] * (float)NY - 0.5f;
		float fz = coords[i * 3 + 2] * (float)NZ - 0.5f;
		...
		for (dz = 0; dz < 2; dz++) for (dy = 0; dy < 2; dy++) for (dx = 0; dx < 2; dx++)
		{
			...
			if (w == 0)
				continue;
			// clamp mirrors TEXF_CLAMP, so an edge midpoint models correctly
			R_Metal_Probe3D_Texel(bound(0, x0 + dx, NX - 1), bound(0, y0 + dy, NY - 1),
			                      bound(0, z0 + dz, NZ - 1), t);
			for (c = 0; c < 4; c++)
				acc[c] += w * (float)t[c];
		}
		// RGBA8Unorm: the shader's .r IS source byte 0, so the channel map is
		// the identity here. Stated rather than assumed because the 2D half of
		// this command uses BGRA, where it is not.
		for (c = 0; c < 4; c++)
		{
			int dev = (int)(fabs(got[i * 4 + c] * 255.0f - acc[c]) + 0.5f);
			if (dev > maxdev) { maxdev = dev; worstsample = i; }
		}
	}
	// 2/255 is generous for an exact midpoint and nowhere near a filter slip: a
	// NEAREST sampler returns an endpoint, which at these steps is 14/255 out.
	Con_Printf("r_metal_textureprobe: 3D sampling %d points, max deviation %d/255 (worst point %d) -- %s\n",
	           n, maxdev, worstsample,
	           maxdev > 2 ? "FAIL" : "PASS (axis order and linear filtering are correct)");
	R_FreeTexturePool(&pool);
}
...
static void GL_TextureMode_f(cmd_state_t *cmd)
{
	...
	{
	case RENDERPATH_METAL:
		// GL re-walks every live texture applying glTexParameteri; Metal keeps
		// filtering in a separate sampler object, so dropping the cache is the
		// whole of the work and the next bind rebuilds what it needs.
...
static void r_textures_start(void)
{
	...
	{
	case RENDERPATH_METAL:
		// Nothing to do: GL needs a global unpack alignment, Metal takes an
		// explicit bytesPerRow on every upload instead.
		break;
	case RENDERPATH_GL32:
	case RENDERPATH_GLES2:
...
void R_Textures_Init (void)
{
	...
	Cmd_AddCommand(CF_CLIENT, "r_metal_textureprobe", &R_Metal_TextureProbe_f, "METAL.md Phase 3: upload a known pattern through R_LoadTexture2D and read it back off the GPU, proving the Metal texture path actually works");
...
static void R_UploadPartialTexture(gltexture_t *glt, const unsigned char *data, int fragx, int fragy, int fragz, int fragwidth, int fragheight, int fragdepth)
{
	...
	case RENDERPATH_METAL:
		if (glt->texnum)
			Metal_Texture_UploadPartial(glt->texnum, data, fragx, fragy, fragwidth, fragheight);
		break;
	case RENDERPATH_GL32:
	case RENDERPATH_GLES2:
		{
...
//		prevbuffer = resizebuffer;
	...
	{
	case RENDERPATH_METAL:
		// Reached from R_SetupTexture unconditionally at load time -- upload is
		// EAGER on both backends. The dirty/R_RealGetTexture deferral applies
		// only to later UPDATES. Note R_UpdateTexture bails early on !texnum,
		// so a texture whose format we refused stays silently un-updatable --
		// which is why the refusal is printed by name at creation.
		//
		// METAL.md Phase 4a: the mip chain is the SAME CPU loop the GL 2D case
		// below runs -- Image_MipReduce32 level by level -- so both backends
		// sample byte-identical mip data. Before this, only level 0 was ever
		// uploaded while the descriptor allocated a full chain and the sampler
		// mip-filtered: every distant world texel read undefined memory.
		// METAL.md Phase 6 (6-1): `depth` is passed. It was dropped before, which
		// was correct only while Metal_Texture_Create refused every volume -- the
		// moment 3D textures were accepted, a 2D-region upload would have written
		// slice 0 of the murk's 64^3 noise volume and left the other 63 undefined.
		// The mip loop already tracks depth for the GL arm below, so it needed
		// nothing new to say.
		if (glt->texnum)
		{
			Metal_Texture_UploadLevel(glt->texnum, mip++, prevbuffer, width, height, depth);
			if (glt->flags & TEXF_MIPMAP)
			{
...
static rtexture_t *R_SetupTexture(rtexturepool_t *rtexturepool, const char *identifier, int width, int height, int depth, int sides, int flags, int miplevel, textype_t textype, int texturetype, const unsigned char *data, const unsigned int *palette)
{
	...
	case TEXTYPE_SHADOWMAP24_RAW:
		break;
	// depth (and packed depth-stencil) as a sampleable TEXTURE rather than a
	// write-only renderbuffer — used by the volumetrics path, which needs to read
	// the camera's scene depth back. These carry no alpha channel to probe, so
	// like the shadowmap types they just fall through with flags unchanged.
	// Without these cases the default: below Sys_Error'd, which is why
	// R_RenderTarget_Get's non-renderbuffer depth branch had never been usable.
	case TEXTYPE_DEPTHBUFFER16:
	case TEXTYPE_DEPTHBUFFER24:
	case TEXTYPE_DEPTHBUFFER24STENCIL8:
		// depth formats are not linearly filterable, so force NEAREST here rather
		// than trusting each caller to pass the right flag
		flags = (flags & ~TEXF_FORCELINEAR) | TEXF_FORCENEAREST;
		break;
	case TEXTYPE_DXT1:
	...
	glt->texnum = 0;
	glt->dirty = false;
	// Packed depth-stencil must be flagged so R_Mesh_CreateFramebufferObject also
	// binds it to GL_STENCIL_ATTACHMENT (gl_backend.c). Renderbuffers already do
	// this (R_LoadTextureRenderBuffer below); textures did not, so a depth-stencil
	// TEXTURE silently lost its stencil — which would break stencil shadow volumes.
	...
	{
	case RENDERPATH_METAL:
		// METAL.md Phase 3: texnum carries an opaque handle into
		// metal_textures.m's table rather than a GL name. It stays an int, so
		// the public R_GetTexture macro and every consumer outside this file
		// are unaffected. 0 means "unsupported format" and metal_textures.m has
		// already said so by name.
		glt->texnum = Metal_Texture_Create(glt->textype->textype, glt->tilewidth, glt->tileheight,
		                                   glt->tiledepth, glt->sides, glt->flags);
		break;
	case RENDERPATH_GL32:
	case RENDERPATH_GLES2:
		CHECKGLERROR
...
rtexture_t *R_LoadTextureRenderBuffer(rtexturepool_t *rtexturepool, const char *identifier, int width, int height, textype_t textype)
{
	...
	{
	case RENDERPATH_METAL:
		// METAL.md Phase 4a: Metal has no renderbuffer/texture split -- an
		// MTLTexture with RenderTarget usage plays both roles -- so the handle
		// goes in texnum, exactly where R_SetupTexture's arm puts it. Nothing
		// on the Metal path ever reads renderbuffernum.
		glt->texnum = Metal_Texture_Create(glt->textype->textype, glt->tilewidth, glt->tileheight, glt->tiledepth, glt->sides, glt->flags);
		break;
	case RENDERPATH_GL32:
	case RENDERPATH_GLES2:
		CHECKGLERROR
...
rtexture_t *R_LoadTextureDDSFile(rtexturepool_t *rtexturepool, const char *filename, qbool srgb, int flags, qbool *hasalphaflag, float *avgcolor, int miplevel, qbool optionaltexture) // DDS textures are opaque, so miplevel isn't a pointer but just seen as a hint
{
	...
	{
	case RENDERPATH_METAL:
		break; // METAL_TODO (METAL.md Phase 4): compressed DDS is world content; the 2D path loads none
	case RENDERPATH_GL32:
	case RENDERPATH_GLES2:
		CHECKGLERROR
		GL_ActiveTexture(0);
