...
// SEPTEMBER2 Part B (2026-09-06): the room, in stereo, behind a master switch. The taste
// rule: Quake's samples are the sound; at 0 the dry path is bit-identical (the mix-dump
// gate), at 1 it stays the loudest thing in the mix. Reverb and filtering are what the
// ROOM does to a sound, never what the sound is.
cvar_t snd_reverb = {CF_CLIENT | CF_ARCHIVE, "snd_reverb", "1", "ROOM REVERB (SEPTEMBER2 B1, 2026-09-06): one algorithmic stereo reverb (eight combs and four allpasses per channel, the Freeverb shape, decorrelated left/right) on a send bus after the mixer. The room is read from the level every frame -- ten tracelines from where you stand give a mean free path and how much of the sky you can see -- so a crypt rings, a corridor is tight and open ground is nearly dry, cross-fading over half a second as you walk between rooms. Each sound's send scales with its distance (far sounds are wetter than near ones, which is most of what 'in the room' means); your own weapon sends a half. Music and menu sounds never enter it. Stereo output only. 0 = every sample plays dry, bit for bit as before. Default 1 since 2026-09-08 (Seb's ear, at wet 0.15)"};
cvar_t snd_reverb_wet = {CF_CLIENT | CF_ARCHIVE, "snd_reverb_wet", "0.15", "snd_reverb: the wet level (0-4) at full closedness (0.15 since 2026-09-07 -- Seb heard 0.3 as \"a bit too glossy\", halved on his word); open sky reduces it toward a sixth. Small numbers: the dry sound must stay the loudest thing in the mix"};
cvar_t snd_reverb_decay = {CF_CLIENT | CF_ARCHIVE, "snd_reverb_decay", "1", "snd_reverb: scale on the room's decay time (0.3-3). At 1 a small room rings ~0.4 s, a big hall ~2 s"};
cvar_t snd_reverb_predelay = {CF_CLIENT | CF_ARCHIVE, "snd_reverb_predelay", "15", "snd_reverb: milliseconds between the direct sound and the first reflection (0-100)"};
cvar_t snd_reverb_lowcut = {CF_CLIENT | CF_ARCHIVE, "snd_reverb_lowcut", "180", "snd_reverb: high-pass on the send in Hz (0 = off): keeps the tail out of the bass so the mix does not muddy"};
cvar_t snd_reverb_senddist = {CF_CLIENT | CF_ARCHIVE, "snd_reverb_senddist", "700", "snd_reverb: the distance in world units at which a sound sends fully into the room; nearer sounds send from a quarter up to that"};
cvar_t snd_reverb_report = {CF_CLIENT, "snd_reverb_report", "0", "snd_reverb: print the room estimate (mean free path, sky share, decay, wet) once a second. Console-only"};
cvar_t snd_occlusion_lowpass = {CF_CLIENT | CF_ARCHIVE, "snd_occlusion_lowpass", "1", "OCCLUSION AS A LOW-PASS (SEPTEMBER2 B2, 2026-09-06): a sound the existing snd_spatialization_occlusion test finds blocked by the world is DULLED as well as quietened -- a one-pole low-pass per channel at snd_occlusion_cutoff, so a Shambler behind a door reads as a muffled presence rather than a distant one. Needs snd_spatialization_occlusion (1, 2 or 3). Mono sounds only; music is never touched. 0 = the old bytes. Default 1 since 2026-09-08 (Seb's ear)"};
cvar_t snd_occlusion_cutoff = {CF_CLIENT | CF_ARCHIVE, "snd_occlusion_cutoff", "1200", "snd_occlusion_lowpass: the cutoff in Hz for a fully blocked sound (200-8000)"};
cvar_t snd_airabsorb = {CF_CLIENT | CF_ARCHIVE, "snd_airabsorb", "0", "DISTANCE AIR ABSORPTION (SEPTEMBER2 B3, 2026-09-10), 0-1: a far sound loses its top end the way a real one does. The same one-pole per channel the occlusion low-pass uses, its cutoff falling with the source's distance from the listener -- nothing within snd_airabsorb_start units is touched at all (bit for bit), and by snd_airabsorb_dist the cutoff has come down to snd_airabsorb_cutoff; the value is how far along that fall the sound is taken (1 = all the way, 0.5 = half the octaves). A blocked sound takes the lower of this and the occlusion cutoff. Mono world sounds only; music and local sounds never. 0 = the old bytes"};
cvar_t snd_airabsorb_start = {CF_CLIENT | CF_ARCHIVE, "snd_airabsorb_start", "300", "snd_airabsorb: the distance (units) under which a sound is never filtered"};
cvar_t snd_airabsorb_dist = {CF_CLIENT | CF_ARCHIVE, "snd_airabsorb_dist", "2000", "snd_airabsorb: the distance (units) at which the cutoff reaches snd_airabsorb_cutoff"};
cvar_t snd_airabsorb_cutoff = {CF_CLIENT | CF_ARCHIVE, "snd_airabsorb_cutoff", "2500", "snd_airabsorb: the cutoff in Hz at snd_airabsorb_dist (200-8000); between start and dist it falls geometrically from the open top end"};
...
// SEPTEMBER2 Part B: THE MIX DUMP (SND_DUMPMIX=<path>). The mixer is CPU and, in a
// timedemo, driven by the demo's own clock (cl.mtime, the "SUPER NASTY HACK" above),
// so under -simsound the stereo mix of a playback is a deterministic function of the
// demo -- which makes it the byte instrument for every sound change: the off switch
// is a byte-identical WAV against a control binary, the on arm a spectrum. Written
// as the device format (16-bit PCM or float), header patched at shutdown.
static FILE *snd_dumpfile;
static unsigned long snd_dumpbytes;
static int snd_dumptried;
void S_DumpMixBytes(const void *bytes, size_t count)
{
	if (!snd_dumptried)
	{
		const char *path = getenv("SND_DUMPMIX");
		snd_dumptried = 1;
		if (path && *path && snd_renderbuffer)
		{
			snd_dumpfile = fopen(path, "wb");
			if (snd_dumpfile)
			{
				unsigned char hdr[44];
				memset(hdr, 0, sizeof(hdr));   // patched in S_Shutdown
				fwrite(hdr, 1, sizeof(hdr), snd_dumpfile);
				Con_Printf("snd: dumping the mix to %s (%d Hz, %d ch, width %d)\n", path, snd_renderbuffer->format.speed, snd_renderbuffer->format.channels, snd_renderbuffer->format.width);
			}
		}
	}
	if (!snd_dumpfile) return;
	fwrite(bytes, 1, count, snd_dumpfile);
	snd_dumpbytes += (unsigned long)count;
}
static void S_DumpMixClose(void)
{
	if (!snd_dumpfile) return;
	{
		unsigned char h[44];
		unsigned int rate = snd_renderbuffer ? (unsigned int)snd_renderbuffer->format.speed : 44100;
		unsigned int ch = snd_renderbuffer ? (unsigned int)snd_renderbuffer->format.channels : 2;
		unsigned int w = snd_renderbuffer ? (unsigned int)snd_renderbuffer->format.width : 2;
		unsigned int fmt = (w == 4) ? 3 : 1, datalen = (unsigned int)snd_dumpbytes, riff = 36 + datalen, brate = rate * ch * w, blk = ch * w, bits = w * 8;
		memcpy(h, "RIFF", 4); memcpy(h + 4, &riff, 4); memcpy(h + 8, "WAVEfmt ", 8);
		{ unsigned int sixteen = 16; memcpy(h + 16, &sixteen, 4); }
		{ unsigned short s = (unsigned short)fmt; memcpy(h + 20, &s, 2); s = (unsigned short)ch; memcpy(h + 22, &s, 2); }
		memcpy(h + 24, &rate, 4); memcpy(h + 28, &brate, 4);
		{ unsigned short s = (unsigned short)blk; memcpy(h + 32, &s, 2); s = (unsigned short)bits; memcpy(h + 34, &s, 2); }
		memcpy(h + 36, "data", 4); memcpy(h + 40, &datalen, 4);
		fseek(snd_dumpfile, 0, SEEK_SET);
		fwrite(h, 1, sizeof(h), snd_dumpfile);
		fclose(snd_dumpfile);
		snd_dumpfile = NULL;
		Con_Printf("snd: mix dump closed, %lu bytes\n", snd_dumpbytes);
	}
}
...
// COMMANDLINEOPTION: Sound: -simsound runs sound mixing but with no output
	...
	Cvar_RegisterVariable(&snd_reverb);
	Cvar_RegisterVariable(&snd_reverb_wet);
	Cvar_RegisterVariable(&snd_reverb_decay);
	Cvar_RegisterVariable(&snd_reverb_predelay);
	Cvar_RegisterVariable(&snd_reverb_lowcut);
	Cvar_RegisterVariable(&snd_reverb_senddist);
	Cvar_RegisterVariable(&snd_reverb_report);
	Cvar_RegisterVariable(&snd_occlusion_lowpass);
	Cvar_RegisterVariable(&snd_occlusion_cutoff);
	Cvar_RegisterVariable(&snd_airabsorb);
	Cvar_RegisterVariable(&snd_airabsorb_start);
	Cvar_RegisterVariable(&snd_airabsorb_dist);
	Cvar_RegisterVariable(&snd_airabsorb_cutoff);
...
static void SND_Spatialize_WithSfx(channel_t *ch, qbool isstatic, sfx_t *sfx)
{
	...
	// SEPTEMBER2 Part B: both arms below assign these; zeroed first so a channel
	// that takes neither (a silent one) never carries a stale send or filter
	ch->sendvol = 0.0f;
	ch->lpalpha = 0.0f;
	...
	{
		// Part B1: your own sounds are never occluded, and send a quarter into the room
		if (snd_reverb.integer && !(ch->flags & CHANNELFLAG_LOCALSOUND))
			ch->sendvol = mastervol * 0.5f;   // a quarter measured 38 dB under the dry: inaudible
	...
	else
	{
		...
		if (intensity > 0)
		{
			...
			if(occluded)
				intensity *= 0.5f;
			// SEPTEMBER2 Part B2: the blocked sound's low-pass coefficient at the
			// SFX's own rate (the mixer filters the fetched samples before it
			// resamples them). 0 = bypass; music and local sounds never qualify.
			if (!(ch->flags & CHANNELFLAG_LOCALSOUND) && sfx && sfx->format.speed > 0)
			{
				// the cutoff this channel takes: the occlusion's for a blocked sound,
				// the air's for a far one, the LOWER of the two when both apply.
				// 0 = no filter (lpalpha stays at the 0 the spatializer reset it to).
				float fc = 0.0f;
				if (occluded && snd_occlusion_lowpass.integer)
					fc = bound(200.0f, snd_occlusion_cutoff.value, 8000.0f);
				// SEPTEMBER2 B3: DISTANCE AIR ABSORPTION. Past snd_airabsorb_start the
				// cutoff falls GEOMETRICALLY (evenly in octaves) from an open 16 kHz
				// to snd_airabsorb_cutoff at snd_airabsorb_dist, scaled by the cvar;
				// under start nothing is touched, so a near sound is the old bytes.
				if (snd_airabsorb.value > 0.0f)
				{
					float start = max(0.0f, snd_airabsorb_start.value);
					float span = max(1.0f, snd_airabsorb_dist.value - start);
					if (dist > start)
					{
						float t = min((dist - start) / span, 1.0f) * bound(0.0f, snd_airabsorb.value, 1.0f);
						float far = bound(200.0f, snd_airabsorb_cutoff.value, 8000.0f);
						float fa = 16000.0f * (float)pow(far / 16000.0, (double)t);
						if (fc <= 0.0f || fa < fc)
							fc = fa;
						{
							static int airreported;
							if (!airreported) { airreported = 1; Con_DPrintf("snd: air absorption armed (first far sound at %.0f units, cutoff %.0f Hz)\n", dist, fa); }
						}
					}
				}
				if (fc > 0.0f)
					ch->lpalpha = 1.0f - (float)exp(-2.0 * M_PI * fc / (double)sfx->format.speed);
			}
			// Part B1: the send -- this sound's level into the room, scaling with its
			// distance from a quarter at point blank to full at snd_reverb_senddist
			if (snd_reverb.integer && !(ch->flags & CHANNELFLAG_LOCALSOUND))
				ch->sendvol = intensity * (0.25f + 0.75f * min(dist / max(1.0f, snd_reverb_senddist.value), 1.0f));
...
============
*/
static void S_UpdateRoom(void)
{
	static float room_free = 400.0f, room_sky = 0.0f;   // the smoothed estimate
	static double lastreport;
	float meanfree, skyshare, room01, closed, rt60, feedback, damp, wet;
	if (!snd_reverb.integer || !cl.worldmodel || !cl.worldmodel->TraceLine)
	{
		S_ReverbSetParams(0, 0, 0, 0, 0, 0);
		return;
	}
	{
		...
		const float reach = 2048.0f;
		float sum = 0.0f; int sky = 0, n;
		trace_t trace;
		vec3_t end;
		for (n = 0; n < 10; n++)
		{
			VectorMA(listener_origin, reach, dirs[n], end);
			memset(&trace, 0, sizeof(trace));
			cl.worldmodel->TraceLine(cl.worldmodel, NULL, NULL, &trace, listener_origin, end, SUPERCONTENTS_SOLID | SUPERCONTENTS_SKY, 0, 0);
			sum += trace.fraction * reach;
			if (trace.hitsupercontents & SUPERCONTENTS_SKY) sky++;
		}
		meanfree = sum / 10.0f;
		skyshare = sky / 10.0f;
	}
	{
		float k = bound(0.0f, (float)cl.realframetime / 0.5f, 1.0f);   // ~0.5 s cross-fade
		room_free += (meanfree - room_free) * k;
		room_sky  += (skyshare - room_sky) * k;
	}
	room01 = bound(0.0f, (room_free - 96.0f) / 900.0f, 1.0f);   // a 96-unit cell to a 1000-unit hall
	closed = 1.0f - room_sky;
	rt60 = (0.4f + 1.6f * room01) * bound(0.3f, snd_reverb_decay.value, 3.0f);
	// a comb of delay d with feedback g decays 60 dB in 3d/(-log10 g) seconds; the
	// EFFECTIVE d of this tank is 0.069 s, MEASURED on the aligned tail-shot bed
	// (2026-09-06: the nominal 0.030 read an RT60 of 1.1 s where 0.5 s was named --
	// the damping filters and the allpasses add tail), so the constant is 0.207
	feedback = (float)pow(10.0, -0.207 / rt60);
	damp = 0.35f + 0.35f * (1.0f - closed);            // open air eats the top end of the tail
	wet = bound(0.0f, snd_reverb_wet.value, 4.0f) * (0.15f + 0.85f * closed);
	S_ReverbSetParams(1, feedback, damp, wet, snd_reverb_predelay.value, snd_reverb_lowcut.value);
	if (snd_reverb_report.integer && host.realtime - lastreport > 1.0)
	{
		lastreport = host.realtime;
		Con_Printf("reverb room: mean free path %.0f units, sky %.0f%%, decay %.2f s (feedback %.3f), wet %.2f\n", room_free, 100.0f * room_sky, rt60, feedback, wet);
	}
}
...
void S_Update(const matrix4x4_t *listenermatrix)
{
	...
	// SEPTEMBER2 Part B1: THE ROOM. Ten tracelines from the listener -- up, down,
	// four compass points, four diagonals -- give a mean free path (how big the
	// space is) and a sky share (how open it is). Decay time follows the size,
	// wet level the closedness, both cross-faded over half a second so walking
	// through a doorway is a change of room, not a switch. Runs on the main
	// thread at frame rate; the mixer takes the result as smoothed targets.
	S_UpdateRoom();
