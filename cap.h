...
#define PBO_COUNT 3 // bones_was_here: slightly faster than double buffering
	...
	GLuint PBOindex;
	/// SEPTEMBER2 E (2026-09-10): the Metal arm reads the screen texture back per frame
	/// (GL row order) and scales it on the CPU; no PBO, no FBO on that path
	unsigned char *metalfull;
	unsigned char *metalscaled;
	GLuint FBO;
	GLuint FBOtex;
}
capturevideostate_t;
