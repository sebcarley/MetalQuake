...
static void Key_BindIfUnbound_f(cmd_state_t *cmd)
{
	int b;
	if (Cmd_Argc(cmd) != 3)
	{
		Con_Print("m5_bindifunbound <key> <command> : bind a key only if it is unbound\n");
		return;
	}
	b = Key_StringToKeynum(Cmd_Argv(cmd, 1));
	if (b == -1)
...
//
	...
	// M5: a bind that yields to the player's own (see host.c, after quake.rc)
	Cmd_AddCommand(CF_CLIENT, "m5_bindifunbound", Key_BindIfUnbound_f, "bind a key ONLY if it is currently unbound: m5_bindifunbound <key> <command>. The M5 engine defaults use it after quake.rc, so id1's unbindall cannot wipe them and a player's config.cfg bind still wins");
}
