...
// Hash a string into all four seed words, each through its own salted FNV-1a
// pass plus an avalanche. Written for deterministic per-map seeding (the fog
// bake): the same name always yields the same four words, different names
// diverge in every word -- INCLUDING the last, which matters because Math_rand64
// derives the bits Math_crandomf reads from s[3] ALONE (the documented frozen-
// curve trap). Never wrap this in something that leaves word 3 constant.
void Math_SeedFromString(unsigned int out[4], const char *s, unsigned int salt)
{
	static const unsigned int wordsalt[4] = {0x9e3779b9u, 0x85ebca6bu, 0xc2b2ae35u, 0x27d4eb2fu};
	int w;
	for (w = 0; w < 4; w++)
	{
		unsigned int h = 2166136261u ^ (salt + wordsalt[w]);
