...
cvar_t vid_vsync = {CF_CLIENT | CF_ARCHIVE, "vid_vsync", "1", "sync to vertical blank, prevents 'tearing' (seeing part of one frame and part of another on the screen at the same time) at the cost of latency, >= 1 always syncs and <= -1 is adaptive (stops syncing if the framerate drops, unsupported by some platforms), automatically disabled when doing timedemo benchmarks. Default 1 since Phase 8-6 QA (2026-08-09, Seb's verdict); test/bench-rt.sh and the timedemo force-off keep benchmarks unaffected"};
...
// M5: renderer backend selection (METAL.md). Read exactly once, in
// VID_InitMode, so a mid-frame change is impossible; applies on vid_restart.
//
// METAL IS THE DEFAULT ON MACOS as of Phase 8 (Seb's call, 2026-08-08): the
// Metal renderpath renders the fork's whole picture at measured parity, RT
// composites natively instead of through the IOSurface bridge, and EDR exists
// only there. The Phase 0 reason for a gl default and a session-only cvar --
// the Metal path drew nothing, so a saved "metal" was a blank window with no
// visible way out -- expired at Phase 3, and its two safeties are replaced by
// better ones: VID_InitMode FALLS BACK to GL when Metal init fails (the window
// never comes up broken), and the cvar is ARCHIVED again, which is now the
// safety rather than the hazard -- with a metal default, "vid_renderer gl"
// must be able to persist or the escape hatch runs backwards.
//
// The DEFAULT is platform-conditional rather than the dispatch quietly
// ignoring it: a metal default on Linux would print a warning every launch.
#ifdef MACOSX
#define VID_RENDERER_DEFAULT "metal"
#else
#define VID_RENDERER_DEFAULT "gl"
#endif
cvar_t vid_renderer = {CF_CLIENT | CF_ARCHIVE, "vid_renderer", VID_RENDERER_DEFAULT, "renderer backend: metal (native Metal, the macOS default -- see METAL.md) or gl (OpenGL 3.2, the reference and the default elsewhere). Takes effect on vid_restart; also on the Video Options menu. If Metal cannot initialise, the engine falls back to GL for that launch on its own"};
...
// The single master brightness control (menu: Options -> Brightness and Gamma).
// It is a MULTIPLIER on the gamma exponent's denominator rather than a cvar the
// menu writes into v_gamma, so v_gamma keeps working as an independent trim and
// nothing on this page silently rewrites anything else on it.
//
// 0.5 is the 1996 look and multiplies by EXACTLY 1.0, so the shipped default is
// bit-for-bit what the engine did before this existed.  Below the midpoint the
// exponent steepens hard -- the whole point is that the bottom of the range is
// nearly unplayable on an OLED, which the old menu floors could not reach: they
// stopped v_gamma at 0.5 and v_contrast at 1.0 while the engine allows 0.1 and
// 0.2.  See VID_BrightnessGammaScale.
// A pack-scoped trim on top of r_brightness, for the per-pack look files. It
// exists so those files never have to touch r_brightness itself: the config is
// now shared across all the games, so a pack writing an archived cvar every
// player cares about would follow you back into Quake the next time you quit.
// Everything a look file sets is m5_* and inert in stock Quake for that reason.
...
void GL_Setup(void)
{
	...
	// explicit, not merely zeroed: GL keeps the old additive paths (F1), so
	// nothing here asks for a max-blended pass and the capability stays off
	vid.blendequationmax = false;
	vid.m5postfx = false;   // the fork's postprocess extras are MSL-only; GL never defines the parm
...
static float cachebrightscale;
static int cachecolorenable;

/*
====================
VID_BrightnessGammaScale

r_brightness 0..1 -> a multiplier on the gamma value (NOT the exponent, which is
its reciprocal), so smaller = darker.  Three straight segments; the two that
meet the 0.5 midpoint are written as an offset FROM 1.0 so the midpoint returns
exactly 1.0f in float arithmetic -- the no-op gate depends on that being exact,
not merely close, since x * 1.0f is bit-preserving and x * 0.99999f is not.

  slider   scale   exponent at v_gamma 1   the brief
    0.00    0.22           4.55            almost unplayable, inky
    0.15    0.55           1.82            very dark and gloomy
    0.25    0.73           1.37            moody but comfortably playable
    0.50    1.00           1.00            the 1996 look, and exactly no change
    1.00    2.00           0.50            clearer and more lit

The lower knee at 0.15 is measured, not guessed.  A single straight line from
0 to the midpoint put 15% and 25% within a whisker of 0% -- 94.6% and 89.0% of
the frame below luminance 8 against 95.3% at zero -- because a power curve
collapses a dark scene long before the slider reaches its end.  Steepening only
the last sixth of the travel keeps 0% genuinely unplayable while leaving the
15-50% stretch usable.
====================
*/
// STOCK MODE's tone reference. r_brightness, v_gamma and v_contrast are ONE
// calibration and you cannot take half of it: overriding the first two alone was
// measured at mean luma 2.07 on the e1m3 spawn against 11.16 for the whole
// chain, because v_gamma carries most of the darkening. A curve calibrated
// against the RT wall-lighting term under-exposes a baked lightmap by roughly
// 8x, so 1996 rendering on a 2026 calibration is a black screen -- which is what
// the first Stock still actually came back as. All three take their DEFAULTS
// here, read-side like everything else in stock, so m5_stock 0 restores the
// player's own curve exactly and nothing of theirs is written.
static float M5_StockGamma(void)    { return m5_stock.integer ? 1.0f : v_gamma.value; }
static float M5_StockContrast(void) { return m5_stock.integer ? 1.0f : v_contrast.value; }

static float VID_BrightnessGammaScale(void)
{
	// STOCK MODE takes r_brightness's own default -- 0.5, which its help calls
	// "the stock 1996 look (and exactly no change)". A player's brightness is
	// normally theirs, but this one is not panel calibration: it scales the gamma
	// curve, and a curve calibrated against the RT wall-lighting term renders a
	// BAKED LIGHTMAP almost black. Measured at Seb's settings on the e1m3 spawn:
	// mean luma 1.35 with the ray tracer off (median 0.0) against 11.16 at the
	// default curve. v_gamma and v_contrast are left entirely alone -- those ARE
	// panel calibration. Read-side like the rest of stock, so nothing is written.
	float t = bound(0.0f, m5_stock.integer ? 0.5f : r_brightness.value, 1.0f);
	float s;
	if (t >= 0.5f)
		s = 1.0f + (t - 0.5f) * 2.0f;
	else if (t >= 0.15f)
		s = 1.0f - (0.5f - t) * (0.45f / 0.35f);
	else
		s = 0.22f + t * ((0.55f - 0.22f) / 0.15f);
	// the per-pack trim, never in stock Quake -- which is the reference the
	// packs are being corrected towards, so there is nothing to correct
	if (gamemode != GAME_NORMAL && m5_packbrightness.value != 1.0f)
		s *= bound(0.25f, m5_packbrightness.value, 4.0f);
	return s;
}
...
void VID_ApplyGammaToColor(const float *rgb, float *out)
{
	...
	{
		for (i = 0; i < 3; i++)
			out[i] = pow(cachecontrastboost * rgb[i] / ((cachecontrastboost - 1) * rgb[i] + 1), 1.0 / (invpow(0.5, 1 - cachegrey[i]) * cachebrightscale)) * cachewhite[i] + cacheblack[i];
	}
	else
	{
		for (i = 0; i < 3; i++)
			out[i] = pow(cachecontrastboost * rgb[i] / ((cachecontrastboost - 1) * rgb[i] + 1), 1.0 / (cachegamma * cachebrightscale)) * cachecontrast + cachebrightness;
	}
}

/*
================
VID_GetGammaAnalytic

METAL.md Phase 7-1. The SAME four quantities BuildGammaTable16 is handed, in the
form a shader wants them: per channel an inverse gamma, a scale and a base, plus
one shared contrast boost. A shader given these evaluates

    t = boost * c / ((boost - 1) * c + 1)
    out = pow(t, invgamma) * scale + base

which is BuildGammaTable16's loop body with the table taken out of the middle,
and is character for character what VID_ApplyGammaToColor above already computes
on the CPU. That is not a coincidence to be tidied away later: it is the reason
this can be validated at all, since the C function is an independent second
implementation of the same formula and the table is a third.

WHY IT MATTERS FOR EDR (Phase 7's actual goal): the table is 256 entries of 8
bits, indexed BY THE COLOUR, so it cannot represent an input above 1.0 and
cannot emit one either -- the clamp is in its construction, not in a line
anybody could delete. The analytic form has no such ceiling, which is what makes
extended-range output expressible at all.

Returns false when the analytic form must NOT be used, and each refusal is a
real difference rather than caution:
  - v_psycho rewrites the ramps with a cosine unrelated to this formula, so
    there is nothing analytic to evaluate. METAL.md schedules psycho to stay on
    the LUT.
  - the sRGB arm post-transforms every table entry through
    Image_sRGBFloatFromLinearFloat AFTER building it, so the table is no longer
...
{
	int i;
	if (v_psycho.integer || vid.sRGB2D || vid.sRGB3D)
		return false;
	*contrastboost = cachecontrastboost;
	for (i = 0; i < 3; i++)
	{
		// LOCKSTEP VID_BuildGammaTables: same two arms, same arguments, in the
		// same order. The gamma passed to BuildGammaTable16 is inverted inside
		// it (invgamma = 1/gamma), which is done here instead.
		if (cachecolorenable)
		{
			invgamma[i] = 1.0f / (invpow(0.5, 1 - cachegrey[i]) * cachebrightscale);
			scale[i]    = cachewhite[i];
			base[i]     = cacheblack[i];
		}
		else
		{
			invgamma[i] = 1.0f / (cachegamma * cachebrightscale);
			scale[i]    = cachecontrast;
			base[i]     = cachebrightness;
		}
	}
	return true;
}
...
{
	// the master multiplies the gamma value in BOTH modes, so it stays live
	// when the per-channel colour controls are enabled
	if (cachecolorenable)
	{
		BuildGammaTable16(1.0f, invpow(0.5, 1 - cachegrey[0]) * cachebrightscale, cachewhite[0], cacheblack[0], cachecontrastboost, ramps, rampsize);
		BuildGammaTable16(1.0f, invpow(0.5, 1 - cachegrey[1]) * cachebrightscale, cachewhite[1], cacheblack[1], cachecontrastboost, ramps + rampsize, rampsize);
		BuildGammaTable16(1.0f, invpow(0.5, 1 - cachegrey[2]) * cachebrightscale, cachewhite[2], cacheblack[2], cachecontrastboost, ramps + rampsize*2, rampsize);
	}
	else
	{
		BuildGammaTable16(1.0f, cachegamma * cachebrightscale, cachecontrast, cachebrightness, cachecontrastboost, ramps, rampsize);
		BuildGammaTable16(1.0f, cachegamma * cachebrightscale, cachecontrast, cachebrightness, cachecontrastboost, ramps + rampsize, rampsize);
		BuildGammaTable16(1.0f, cachegamma * cachebrightscale, cachecontrast, cachebrightness, cachecontrastboost, ramps + rampsize*2, rampsize);
	}
...
#undef BOUNDCVAR
	...
	// the master scales the gamma value in both modes, so it has to be neutral
	// for the identity table to be identity
	if(VID_BrightnessGammaScale() == 1.0f)
	if(!vid.sRGB2D)
	if(!vid.sRGB3D)
	{
		...
		{
			// STOCK MODE's tone reference (see M5_StockGamma/M5_StockContrast):
			// tested here too, so the trivial-tables test and the tables it is
			// testing can never disagree about which curve is in force.
			if(M5_StockGamma() == 1)
			if(M5_StockContrast() == 1)
...
#define GAMMACHECK(cache, value) if (cache != (value)) gamma_changed = true;cache = (value)
	if(v_psycho.integer)
		gamma_changed = true;
	GAMMACHECK(cachebrightscale, VID_BrightnessGammaScale());
	GAMMACHECK(cachegamma      , M5_StockGamma());
	GAMMACHECK(cachecontrast   , M5_StockContrast());
...
{
	// METAL.md Phase 7-3. The floor, set once and before anything can read it.
	// vid is a file-scope global so it starts zeroed, and a zero here would
	// reach the shoulder and the gamma ceiling as "no headroom at all" -- both
	...
	Cvar_RegisterVariable(&m5_packbrightness);
