...
{ GAME_NORMAL,					GAME_NORMAL,				"",						"-quake",					"DarkPlaces-Quake",			"DarkPlaces-Quake",			"id1",		"m5",			"dp",				"darkplaces"			}, // COMMANDLINEOPTION: Game: -quake runs the game Quake (default; this fork auto-mounts the m5 gamedir so the M5 QuakeC mods need no -game argument)
...
// M5: the two rerelease add-ons. Both are id1-based and use only stock builtins
// (their shared progs.dat calls nothing above builtin #90 plus #99
// checkextension), so they need no special handling -- these rows exist so they
// identify themselves honestly and get their own screenshot prefix instead of
// falling through to plain Quake.
{ GAME_DOPA,					GAME_NORMAL,				"dopa",					"-dopa",					"Dimension of the Past",	"Dimension_of_the_Past",	"id1",		"dopa",			"dopa",				"darkplaces"			}, // COMMANDLINEOPTION: Game: -dopa runs the add-on Dimension of the Past
{ GAME_MG1,						GAME_NORMAL,				"mg1",					"-mg1",						"Dimension of the Machine",	"Dimension_of_the_Machine",	"id1",		"mg1",			"mg1",				"darkplaces"			}, // COMMANDLINEOPTION: Game: -mg1 runs the add-on Dimension of the Machine
};
...
int COM_ChangeGameTypeForGameDirs(unsigned numgamedirs, const char *gamedirs[], qbool failmissing, qbool init)
{
	...
	// First check if a base game matches. Upstream looked for a group member
	// with no SECOND gamedir, which was the same thing as "the group's base
	// game" only because every base game happened to have one gamedir.
	//
	// M5: this fork gives GAME_NORMAL a second gamedir (m5), so that test
	// skipped it and landed on the first other qualifying row in the group --
	// GAME_OPENQUARTZ. The visible effect was that "gamedir id1", which is
	// exactly what the Browse Mods page sends when nothing is ticked, dropped
	// the game into OpenQuartz mode and silently unmounted m5, with no way
	// back short of typing "gamedir m5". Match on the group's own mode
	// instead, which is what the loop was always reaching for.
	for (i = 0; i < gamemode_count; i++)
	{
		if (gamemode_info[i].group == com_startupgamegroup && gamemode_info[i].mode == com_startupgamegroup)
		{
			index = i;
			break;
		}
	...
	//
	// M5: the third clause. Upstream identified a base game by its FIRST
	// gamedir only for rows that have no second one, so with m5 attached to
	// GAME_NORMAL the name "id1" matched no row until it reached
	// GAME_OPENQUARTZ, whose first gamedir is also id1 -- and that is a
	// different game, with a different name, a different screenshot prefix and
	// no m5. Matching the group's own base game on its first gamedir fixes it,
	// and cannot steal a match from a mod row because those are identified by
	// their SECOND gamedir, which a base game's first never collides with.
	for (j = numgamedirs - 1; j >= 0; --j)
	{
		for (i = 0; i < gamemode_count; i++)
		{
			...
			|| (gamemode_info[i].mode == com_startupgamegroup && !strcasecmp(gamedirs[j], gamemode_info[i].gamedirname1)))
			{
				index = i;
				goto double_break;
			}
