...
// WATERSURFACE (r_watersurface): THE POOL FLOOR. On a map without transparent-water
// vis the air's PVS never reaches the leaves below a water surface, so a pool's
// interior is culled and a screen-space refraction has nothing but murk to bend --
// which is what Seb saw, and what FTE does not: FTE renders its refraction as a
// second scene with the PVS of the leaf 0.1 units BELOW each visible water surface
// (its gl_rmain.c GLR_DrawPortal, forcedvis), so the floor is always in its image.
// The same PVS merged into THIS view's bits draws the interior into the frame the
// copy is taken from -- lit by the RT term and fogged by the murk like any floor --
// for the cost of the surfaces themselves and one FatPVS merge per pool. The leaf
// below is found from the face's own normal, tested on both sides and taken only if
// it is liquid, because a side flag is not something this loader promises.
extern cvar_t r_watersurface;
extern cvar_t r_wateralpha_force;
extern cvar_t r_water;
static void R_View_WorldVisibility_WaterFloor(model_t *model)
{
	int i, j, k, *mark, merged = 0;
	const msurface_t *surface;
	mleaf_t *leaf;
	const mleaf_t *mergedleafs[64];
	for (i = model->submodelsurfaces_start, surface = model->data_surfaces + i; i < model->submodelsurfaces_end; i++, surface++)
	{
		vec3_t centre, under;
		const float *normal;
		const mleaf_t *below;
		if (!r_refdef.viewcache.world_surfacevisible[i])
			continue;
		if (!surface->texture || !(surface->texture->basematerialflags & MATERIALFLAG_WATERALPHA) || (surface->texture->basematerialflags & MATERIALFLAG_SKY))
			continue;
		VectorMAM(0.5f, surface->mins, 0.5f, surface->maxs, centre);
		normal = model->surfmesh.data_normal3f + 3 * surface->num_firstvertex;
		VectorMA(centre, -1.0f, normal, under);
		if (!(model->PointSuperContents(model, 0, under) & SUPERCONTENTS_LIQUIDSMASK))
		{
			VectorMA(centre, 1.0f, normal, under);
			if (!(model->PointSuperContents(model, 0, under) & SUPERCONTENTS_LIQUIDSMASK))
				continue;
		}
		below = model->brush.PointInLeaf(model, under);
		if (!below || below->clusterindex < 0)
			continue;
		for (k = 0; k < merged; k++)
			if (mergedleafs[k] == below)
				break;
		if (k < merged)
			continue;
		if (merged >= (int)(sizeof(mergedleafs) / sizeof(mergedleafs[0])))
			break;
		mergedleafs[merged++] = below;
		model->brush.FatPVS(model, under, 2, &r_refdef.viewcache.world_pvsbits, r_main_mempool, true);
	}
	if (!merged)
		return;
	// mark what the merged bits reach and the frustum admits, exactly as the pvs
	// method does; leaves already visible are untouched
	for (j = 0, leaf = model->brush.data_leafs; j < model->brush.num_leafs; j++, leaf++)
	{
		if (leaf->clusterindex < 0 || r_refdef.viewcache.world_leafvisible[j])
			continue;
...
void R_View_WorldVisibility(qbool forcenovis)
{
	...
	// WATERSURFACE: the pool floor -- gated on the feature and on the water actually
	// rendering blended (R_GetCurrentTexture's own enabling set), so at 0 the visible
	// set is byte for byte what it was. See R_View_WorldVisibility_WaterFloor.
	if (r_watersurface.integer && r_wateralpha.value < 1.0f && !r_lockpvs.integer && !r_refdef.view.usecustompvs
	 && (model->brush.supportwateralpha || r_wateralpha_force.integer || r_water.integer || r_novis.integer || r_trippy.integer)
	 && model->brush.FatPVS && model->brush.PointInLeaf && model->PointSuperContents)
		R_View_WorldVisibility_WaterFloor(model);
