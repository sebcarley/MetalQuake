...
cvar_t v_idlescale = {CF_CLIENT, "v_idlescale", "0", "how much of the quake 'drunken view' effect to use, applied constantly. v_idlesway drives the same machinery only while you are standing still"};

// ---------- IDLE VIEW SWAY ----------
// Quake shipped the whole sway apparatus -- the three sin() terms above, their
// cycles and their levels -- and then set v_idlescale to 0 for release, which
// left nothing in the engine that ever wrote it.  Romero has described the
// effect as having been switched off rather than removed, and this is what he
// means: the code is all still here.  Original Quake did use it in one place,
// V_CalcIntermissionRefdef, which forced v_idlescale to 1 for the end-of-level
// camera; DarkPlaces dropped that too when it restructured the function.
//
// This reinstates it as a RAMPED idle behaviour rather than a constant one:
// stand still, stop looking around, and after v_idlesway_delay the view eases
// into the same drift, then eases straight back out the moment you touch
// anything.  v_idlescale still works exactly as before and the larger of the
// two wins, so a player who wants the permanent drunken view still has it.
//
// It is cosmetic by construction.  The sway is added to the LOCAL viewangles
// used to build the view matrix, never to cl.viewangles, which is what the
// server receives -- so it cannot move your aim.
cvar_t v_idlesway = {CF_CLIENT | CF_ARCHIVE, "v_idlesway", "1", "sway the view gently once you have stood still and stopped moving the mouse for a moment, the way the end-of-level camera does. 0 restores the dead-still view"};
cvar_t v_idlesway_delay = {CF_CLIENT | CF_ARCHIVE, "v_idlesway_delay", "1.5", "seconds of standing still, with no buttons and no mouse movement, before the sway starts"};
cvar_t v_idlesway_ramp = {CF_CLIENT | CF_ARCHIVE, "v_idlesway_ramp", "2", "seconds the sway takes to ease in to full strength. It always eases OUT in a quarter of a second, so it never fights your input"};
cvar_t v_idlesway_scale = {CF_CLIENT | CF_ARCHIVE, "v_idlesway_scale", "0.6", "strength of the idle sway relative to the classic v_idlescale 1 drunken view. The shape comes from v_ipitch/iyaw/iroll_cycle and _level"};
...
The effective sway strength for this frame: the larger of the classic constant
v_idlescale and the ramped idle amount.  Advances the ramp, so it must be called
exactly once per V_CalcRefdefUsing.
==================
*/
static float V_IdleSwayScale (float smoothtime, const vec3_t clviewangles, const vec3_t clvelocity, qbool cldead)
{
	float target, step;
	qbool busy;

	if (v_idlesway.value <= 0)
	{
		cl.idlesway_idletime = 0;
		cl.idlesway_ramp = 0;
		VectorCopy(clviewangles, cl.idlesway_prevangles);
		return v_idlescale.value;
	}

	// "busy" is anything the player is doing.  Buttons count, so holding fire
	// at a wall keeps the view dead still; velocity counts, so being carried by
	// a platform or a current does too.  The angle test catches the mouse: a
	// stationary mouse produces an exactly zero delta, and the epsilon is only
	// there to absorb V_DriftPitch's slow pitch centring.
	busy = cl.cmd.forwardmove != 0 || cl.cmd.sidemove != 0 || cl.cmd.upmove != 0
		|| cl.cmd.buttons != 0
		|| DotProduct(clvelocity, clvelocity) > 4.0f
		|| fabs(clviewangles[PITCH] - cl.idlesway_prevangles[PITCH]) > 0.05f
		|| fabs(clviewangles[YAW]   - cl.idlesway_prevangles[YAW])   > 0.05f
		|| fabs(clviewangles[ROLL]  - cl.idlesway_prevangles[ROLL])  > 0.05f;
	VectorCopy(clviewangles, cl.idlesway_prevangles);

	// photo mode flies its own camera and the sway would fight it
	if (busy || cldead || m5_photomode.integer)
		cl.idlesway_idletime = 0;
	else
		cl.idlesway_idletime += smoothtime;

	// target is only ever 0 or 1, so the sign of step follows it directly
	target = cl.idlesway_idletime > v_idlesway_delay.value ? 1.0f : 0.0f;
	if (target > 0.0f)
		step = v_idlesway_ramp.value > 0 ? smoothtime / v_idlesway_ramp.value : 1.0f;
	else
		step = -smoothtime * 4.0f;   // always out in a quarter of a second
	cl.idlesway_ramp = bound(0.0f, cl.idlesway_ramp + step, 1.0f);

	return max(v_idlescale.value, v_idlesway.value * v_idlesway_scale.value * cl.idlesway_ramp);
}
...
The three sin() terms Quake has always carried, kept in one place now that two
callers want them.
==================
*/
static void V_AddIdleSway (vec3_t viewangles, float scale)
{
	if (!scale)
		return;
	viewangles[PITCH] += scale * sin(cl.time * v_ipitch_cycle.value) * v_ipitch_level.value;
	viewangles[YAW]   += scale * sin(cl.time * v_iyaw_cycle.value)   * v_iyaw_level.value;
	viewangles[ROLL]  += scale * sin(cl.time * v_iroll_cycle.value)  * v_iroll_level.value;
}
...
static void V_CalcIntermissionRefdef (vec3_t vieworg, vec3_t viewangles, const matrix4x4_t *entrendermatrix, float clstatsviewheight)
{
	...
	float sway = max(v_idlescale.value, v_idlesway.value);
	...
	// Original Quake forced v_idlescale to 1 here -- the end-of-level camera
	// always drifted, and it is the reference for the idle sway everywhere
	// else.  DarkPlaces lost it when this function stopped going through
	// viewangles, so the sway is applied to the matrix instead: post-
	// multiplying by a pure rotation turns the camera in its OWN frame, which
	// is what adding to its angles used to do.
	if (sway > 0)
	{
		vec3_t swayangles;
		matrix4x4_t swaymatrix, base;
		VectorClear(swayangles);
		V_AddIdleSway(swayangles, sway);
		Matrix4x4_CreateFromQuakeEntity(&swaymatrix, 0, 0, 0, swayangles[PITCH], swayangles[YAW], swayangles[ROLL], 1);
		base = r_refdef.view.matrix;
		Matrix4x4_Concat(&r_refdef.view.matrix, &base, &swaymatrix);
	}
...
void V_CalcRefdefUsing (const matrix4x4_t *entrendermatrix, const vec3_t clviewangles, qbool teleported, qbool clonground, qbool clcmdjump, float clstatsviewheight, qbool cldead, const vec3_t clvelocity)
{
	...
	float swayscale = 0;	// idle view sway, computed once and applied to the view and the gun separately
	matrix4x4_t tmpmatrix;
	...
	else
	{
		...
		else
		{
			...
			// Work out this frame's sway ONCE here (the call advances the ramp,
			// so it must not run twice), and apply it in two places further
			// down: to gunangles once it is final, and to viewangles at the
			// very end.  It cannot simply be added to viewangles here, because
			// viewangles is REBUILT from clviewangles further down, after the
			// gun block -- which is also why the stock v_idlescale had to sit
			// at the very end, and why enabling it moved the world while
			// leaving the weapon pinned to the screen.
			swayscale = V_IdleSwayScale(smoothtime, clviewangles, clvelocity, cldead);
			...
			if (!cldead)
			{
				...
				// The gun's half of the idle sway. viewangles gets its own copy
				// at the very end of the function, after it is rebuilt from
				// clviewangles; each therefore carries exactly one sway and the
				// weapon stays welded to the view instead of sliding across it.
				V_AddIdleSway(gunangles, swayscale);
		...
		// The view's half of the idle sway. It has to be here, at the very end:
		// viewangles is rebuilt from clviewangles after the gun block, so
		// anything added earlier is discarded. gunangles got the same amount
		// back there, before that rebuild.
		V_AddIdleSway(viewangles, swayscale);
...
Self-test for the idle sway's ramp and envelope, so the behaviour can be
asserted headlessly rather than eyeballed.  Steps the ramp with a synthetic
frame time -- first idle, so it must reach full strength shortly after the
delay, then busy, so it must collapse inside the quarter-second ease-out --
and reports the peak angular excursion the envelope can produce.  Restores the
live ramp state afterwards so running it mid-game does not disturb the view.
=============
*/
static void V_IdleSway_Test_f(cmd_state_t *cmd)
{
	double saveidle = cl.idlesway_idletime;
	float saveramp = cl.idlesway_ramp;
	const float dt = 1.0f / 60.0f;
	float t, peak = 0.0f;
	int i, risesteps = 0, fallsteps = 0;

	// rise: idle, so the ramp should pass the delay and then climb to 1
	cl.idlesway_idletime = 0;
	cl.idlesway_ramp = 0;
	for (i = 0; i < 60 * 20 && cl.idlesway_ramp < 1.0f; i++)
	{
		cl.idlesway_idletime += dt;
		if (cl.idlesway_idletime > v_idlesway_delay.value)
			cl.idlesway_ramp = bound(0.0f, cl.idlesway_ramp + (v_idlesway_ramp.value > 0 ? dt / v_idlesway_ramp.value : 1.0f), 1.0f);
		risesteps++;
	}
	// fall: busy, so the ease-out rate applies
	for (i = 0; i < 60 * 20 && cl.idlesway_ramp > 0.0f; i++)
	{
		cl.idlesway_ramp = bound(0.0f, cl.idlesway_ramp - dt * 4.0f, 1.0f);
		fallsteps++;
	}
	// envelope: the largest excursion the three sin terms can reach together
	for (i = 0; i < 4000; i++)
	{
		t = i * 0.001f;
		peak = max(peak, (float)(fabs(sin(t * v_ipitch_cycle.value) * v_ipitch_level.value)
			+ fabs(sin(t * v_iyaw_cycle.value) * v_iyaw_level.value)
			+ fabs(sin(t * v_iroll_cycle.value) * v_iroll_level.value)));
	}
	peak *= v_idlesway.value * v_idlesway_scale.value;

	Con_Printf("M5 sway: rise %.2fs (delay %.2f + ramp %.2f), fall %.2fs, peak %.3f deg -- %s\n",
		risesteps * dt, v_idlesway_delay.value, v_idlesway_ramp.value, fallsteps * dt, peak,
		(risesteps * dt <= v_idlesway_delay.value + v_idlesway_ramp.value + 0.1f
		 && fallsteps * dt <= 0.30f && peak < 2.0f) ? "envelope holds" : "ENVELOPE FAILED");

	cl.idlesway_idletime = saveidle;
	cl.idlesway_ramp = saveramp;
}
...
void V_Init (void)
{
	Cmd_AddCommand(CF_CLIENT, "v_idlesway_test", V_IdleSway_Test_f, "step the idle view sway's ramp headlessly and report the rise time, fall time and peak angular excursion");
	...
	Cvar_RegisterVariable (&v_idlesway);
	Cvar_RegisterVariable (&v_idlesway_delay);
	Cvar_RegisterVariable (&v_idlesway_ramp);
	Cvar_RegisterVariable (&v_idlesway_scale);
