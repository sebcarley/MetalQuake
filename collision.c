...
// ==========================================================================
// Ray-cast core parity + throughput harness (read-only diagnostics).
//
// Fires a deterministic, seeded set of point/box/surface traces against a
// model's collision and writes full-precision results to a text file. Diffing
// the file before vs after a refactor of the trace core (BIH traversal, the
// ray/triangle test, etc.) proves the change is bit-identical -- the safety net
// for gameplay-critical physics, which shares this exact code. The reported
// traces/sec is also the micro-benchmark for those same optimizations.
//
// The three trace kinds cover distinct code paths: TraceLine and TraceBox hit
// the world hull / BIH box path, while TraceLineAgainstSurfaces exercises the
// per-triangle intersection path that the bouncegrid and lightmap ray tracers
// also use. Seeded via the deterministic Lehmer RNG so runs are reproducible.
static unsigned int Collision_ParityHash(unsigned int hash, const void *data, size_t len)
{
	const unsigned char *p = (const unsigned char *)data;
	size_t i;
	for (i = 0; i < len; i++)
		hash = (hash ^ p[i]) * 16777619u; // FNV-1a
	return hash;
}

void Collision_TraceParityTest(struct model_s *model, int numtraces, unsigned int seed, const char *outname)
{
	randomseed_t rng;
	qfile_t *f;
	trace_t trace;
	int i, kind, ntraces = 0;
	vec3_t mins, maxs, size, start, end;
	const vec3_t boxmins = {-16, -16, -24}; // standard Quake player hull
	const vec3_t boxmaxs = { 16,  16,  32};
	const int mask = SUPERCONTENTS_SOLID;
	unsigned int hash = 2166136261u; // FNV-1a offset basis
	double t0, t1, secs;

	if (!model || (!model->TraceLine && !model->TraceBox && !model->TraceLineAgainstSurfaces))
	{
		Con_Printf("collision_dumptraces: model has no collision trace functions\n");
		return;
	}
	if (numtraces < 1)
		numtraces = 1;

	Math_RandomSeed_FromInts(&rng, seed, seed ^ 0x9e3779b9u, seed + 0x243f6a88u, (seed * 2654435761u) | 1u);

	// sample within the model bounds, slightly expanded so some rays start outside
	VectorCopy(model->normalmins, mins);
	VectorCopy(model->normalmaxs, maxs);
	VectorSubtract(maxs, mins, size);
	VectorMA(mins, -0.05f, size, mins);
	VectorMA(maxs,  0.05f, size, maxs);

	f = FS_OpenRealFile(outname, "w", false);
	if (!f)
	{
		Con_Printf("collision_dumptraces: could not open \"%s\" for writing\n", outname);
		return;
	}
	FS_Printf(f, "# collision trace parity dump\n");
	FS_Printf(f, "# model \"%s\" numtraces %d seed %u mask %d\n", model->name, numtraces, seed, mask);
	FS_Printf(f, "# kind fraction endpos.xyz normal.xyz dist startsolid allsolid hitsupercontents hitq3surfaceflags\n");

	t0 = Sys_DirtyTime();
	for (i = 0; i < numtraces; i++)
	{
		start[0] = Math_randomrangef(&rng, mins[0], maxs[0]);
		start[1] = Math_randomrangef(&rng, mins[1], maxs[1]);
		start[2] = Math_randomrangef(&rng, mins[2], maxs[2]);
		end[0]   = Math_randomrangef(&rng, mins[0], maxs[0]);
		end[1]   = Math_randomrangef(&rng, mins[1], maxs[1]);
		end[2]   = Math_randomrangef(&rng, mins[2], maxs[2]);
		for (kind = 0; kind < 3; kind++)
		{
			memset(&trace, 0, sizeof(trace));
			trace.fraction = 1;
			if (kind == 0 && model->TraceLine)
				model->TraceLine(model, NULL, NULL, &trace, start, end, mask, 0, 0);
			else if (kind == 1 && model->TraceBox)
				model->TraceBox(model, NULL, NULL, &trace, start, boxmins, boxmaxs, end, mask, 0, 0);
			else if (kind == 2 && model->TraceLineAgainstSurfaces)
				model->TraceLineAgainstSurfaces(model, NULL, NULL, &trace, start, end, mask, 0, 0);
			else
				continue;
			ntraces++;
			hash = Collision_ParityHash(hash, &trace.fraction, sizeof(trace.fraction));
			hash = Collision_ParityHash(hash, trace.endpos, sizeof(trace.endpos));
			hash = Collision_ParityHash(hash, trace.plane.normal, sizeof(trace.plane.normal));
			hash = Collision_ParityHash(hash, &trace.plane.dist, sizeof(trace.plane.dist));
			hash = Collision_ParityHash(hash, &trace.hitsupercontents, sizeof(trace.hitsupercontents));
			hash = Collision_ParityHash(hash, &trace.hitq3surfaceflags, sizeof(trace.hitq3surfaceflags));
			FS_Printf(f, "%c %.17g %.17g %.17g %.17g %.9g %.9g %.9g %.9g %d %d %d %d\n",
				kind == 0 ? 'L' : (kind == 1 ? 'B' : 'S'),
				...
				trace.startsolid ? 1 : 0, trace.allsolid ? 1 : 0,
				trace.hitsupercontents, trace.hitq3surfaceflags);
		}
	}
	t1 = Sys_DirtyTime();
	secs = t1 - t0;
	if (secs < 0)
		secs = 0; // guard against a timer discontinuity

	FS_Printf(f, "# hash %08x traces %d\n", hash, ntraces);
	FS_Close(f);

	Con_Printf("collision_dumptraces: %d traces on \"%s\", hash %08x, %.1f ms (%.0f traces/sec) -> %s\n",
		ntraces, model->name, hash, secs * 1000.0, secs > 0 ? ntraces / secs : 0.0, outname);
}

void Collision_ExportWorldTriangles(struct model_s *model, const char *outname)
{
	qfile_t *f;
	int i;
	const surfmesh_t *m;
	...
	if (!model)
	{
		Con_Printf("collision_exportworldtris: no model\n");
		return;
	}
	m = &model->surfmesh;
	if (m->num_vertices < 1 || m->num_triangles < 1 || !m->data_vertex3f || !m->data_element3i)
	{
		Con_Printf("collision_exportworldtris: model \"%s\" has no surface mesh\n", model->name);
		return;
	}
	f = FS_OpenRealFile(outname, "w", false);
	if (!f)
	{
		Con_Printf("collision_exportworldtris: could not open \"%s\" for writing\n", outname);
		return;
	}
	FS_Printf(f, "# darkplaces world triangle export\n");
	FS_Printf(f, "# model \"%s\"\n", model->name);
	FS_Printf(f, "verts %d\n", m->num_vertices);
	FS_Printf(f, "tris %d\n", m->num_triangles);
	for (i = 0; i < m->num_vertices; i++)
		FS_Printf(f, "v %.9g %.9g %.9g\n", m->data_vertex3f[i*3+0], m->data_vertex3f[i*3+1], m->data_vertex3f[i*3+2]);
	for (i = 0; i < m->num_triangles; i++)
		FS_Printf(f, "t %d %d %d\n", m->data_element3i[i*3+0], m->data_element3i[i*3+1], m->data_element3i[i*3+2]);
	FS_Close(f);
	Con_Printf("collision_exportworldtris: %d verts, %d tris from \"%s\" -> %s\n", m->num_vertices, m->num_triangles, model->name, outname);
}
