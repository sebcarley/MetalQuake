...
qbool OGG_OpenLibrary (void)
{
	const char* dllnames_vo [] =
	{
		...
		"vorbis.dll",
#elif defined(MACOSX)
		// The bare leaf name first, so anything that already works is unchanged.
		// macOS resolves a bare name only through DYLD_LIBRARY_PATH (and the
		// FALLBACK path, which is $HOME/lib:/usr/local/lib:/usr/lib) -- and on a
		// Homebrew machine libvorbis lives in /opt/homebrew/lib, which is on
		// NEITHER. The Xcode scheme exports DYLD_LIBRARY_PATH, so music worked
		// under Cmd+R and was silent through every other launch route, with the
		// only evidence a developer-only " - failed." line: OGG_OpenLibrary's
		// return value is discarded (snd_main.c), there is no built-in Vorbis
		// decoder, and every .ogg then fails with music's own complain flag off.
		// The absolute fallbacks below make the decoder launch-route-independent.
		// Beside the executable FIRST of the absolute forms: a packaged .app
		// carries its own copy in Contents/MacOS, and @executable_path is the
		// only spelling that finds it on a machine with no Homebrew at all --
		// which is every machine the release zip is opened on.
		"@executable_path/libvorbis.dylib",
...
#elif defined(MACOSX)
		"libvorbisfile.dylib",
		// LOCKSTEP with dllnames_vo above -- see the note there.
		"@executable_path/libvorbisfile.dylib",
