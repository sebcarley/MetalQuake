...
extern cvar_t m5_stock;   // r_shadow.c -- lives there because THIS file links into the dedicated server
...
cvar_t m5_liquidflags = {CF_CLIENT | CF_SERVER | CF_ARCHIVE, "m5_liquidflags", "1", "keep a Q1 liquid texture's own material flags and contents when a replacement texture pack supplies its image. Without this an external *water/*slime/*lava image is loaded as a plain opaque wall, silently disabling r_wateralpha, rt_metal_liquids, the lava glow/boil and the RT lava instance, and making liquids cast shadows. 0 restores the old behaviour exactly"};
...
//	Cvar_RegisterVariable(&r_subdivide_size);
	...
	Cvar_RegisterVariable(&m5_liquidflags);
...
// M5: a Q1 liquid texture is identified by its NAME, and that classification has
// to happen wherever the image came from. These two helpers are the single
// definition of it -- the in-tree loader below calls them, and so does the
// external-texture arm that used to skip the whole thing (see m5_liquidflags).
//
// Both take a name rather than reading tx->name, because the external-texture
// caller has no usable tx->name: Mod_LoadTextureFromQ3Shader overwrites it with
// whichever lookup key succeeded, which for the map-qualified one is
// "e1m3/*04water1" -- no *lava or *slime prefix test would match that.
static void Mod_Q1BSP_LiquidContentsFromName(texture_t *tx, const char *name)
{
	if (!strncmp(name, "*lava", 5))
	{
		tx->supercontents = mod_q1bsp_texture_lava.supercontents;
		tx->surfaceflags = mod_q1bsp_texture_lava.surfaceflags;
	}
	else if (!strncmp(name, "*slime", 6))
	{
...
// the material bits a '*' texture ORs on top of MATERIALFLAG_WALL
static int Mod_Q1BSP_LiquidMaterialFlagsFromName(const char *name)
{
	if (!strncmp(name, "*glassmirror", 12)) // Tenebrae
		return MATERIALFLAG_NOSHADOW | MATERIALFLAG_ADD | MATERIALFLAG_BLENDED | MATERIALFLAG_REFLECTION;
	// LadyHavoc: some turbulent textures should not be affected by wateralpha
	if (!strncmp(name, "*lava", 5))
		return MATERIALFLAG_WATERSCROLL | MATERIALFLAG_LIGHTBOTHSIDES | MATERIALFLAG_NOSHADOW;
	if (!strncmp(name, "*teleport", 9)
	 || !strncmp(name, "*rift", 5)) // Scourge of Armagon texture
		// M5 (F3): same flags as lava plus the teleport marker, which is what
		// r_teleportswirl keys its starfield churn on. Splitting the arm changes
		// nothing for *lava.
		return MATERIALFLAG_WATERSCROLL | MATERIALFLAG_LIGHTBOTHSIDES | MATERIALFLAG_NOSHADOW | MATERIALFLAG_M5TELEPORT;
	return MATERIALFLAG_WATERSCROLL | MATERIALFLAG_LIGHTBOTHSIDES | MATERIALFLAG_NOSHADOW | MATERIALFLAG_WATERALPHA | MATERIALFLAG_WATERSHADER;
}
...
static void Mod_Q1BSP_LoadTextures(sizebuf_t *sb)
{
	...
	for (i = 0;i < nummiptex;i++)
	{
		                ...
		                Mod_LoadTextureFromQ3Shader(loadmodel->mempool, loadmodel->name, loadmodel->data_textures + i, va(vabuf, sizeof(vabuf), "%s"   , name), false, false, TEXF_ALPHA | TEXF_MIPMAP | TEXF_ISWORLD | TEXF_PICMIP | TEXF_COMPRESS, MATERIALFLAG_WALL)))
		{
			...
			// plain opaque wall. Measured on e1m3 with QRP installed: *04water1
			// came out basematerialflags 0x40 (WALL alone) against 0x44550
			// without the pack, and *lava1 came out supercontents SOLID|OPAQUE
			// instead of LAVA. That silently killed r_wateralpha and
			// r_wateralpha_force, rt_metal_liquids, r_lavaglow, r_lavaboil, the
			// rt_metal_lavaemissive instance and the lava lights, put liquids
			// into the RT world BLAS as shadow casters (they lost NOSHADOW), and
			// stopped them scrolling. It is upstream behaviour, not ours.
			//
			// Only the '*' liquids need this: the condition above excludes "sky"
			// by name, so sky never reaches here. Only re-apply when there is no
			// real q3 shader for the name -- a shader that DOES exist authored
			// its surfaceparms deliberately and must keep them. The image's own
			// contribution (an alpha channel adding ALPHA|BLENDED|NOSHADOW) is
			// already in basematerialflags and is preserved by ORing.
			if (m5_liquidflags.integer && name[0] == '*'
			 && !Mod_LookupQ3Shader(va(vabuf, sizeof(vabuf), "%s/%s", mapname, name))
			 && !Mod_LookupQ3Shader(name))
			{
				Mod_Q1BSP_LiquidContentsFromName(tx, name);
				tx->basematerialflags |= Mod_Q1BSP_LiquidMaterialFlagsFromName(name);
		...
		if (tx->name[0] == '*')
			Mod_Q1BSP_LiquidContentsFromName(tx, tx->name);
		...
		if (tx->name[0] == '*')
		{
			tx->basematerialflags |= Mod_Q1BSP_LiquidMaterialFlagsFromName(tx->name);
	...
	// M5 (F3): one summary line per map so a bed can assert the teleport
	// classification fired -- the "M5 maplights" shape, developer-gated,
	// counted by a post-scan so it covers the external-texture arm and the
	// classic arm alike
	{
		int m5teleportcount = 0;
		for (i = 0; i < loadmodel->num_textures; i++)
			if (loadmodel->data_textures && (loadmodel->data_textures[i].basematerialflags & MATERIALFLAG_M5TELEPORT))
				m5teleportcount++;
		if (m5teleportcount)
			Con_DPrintf("M5 teleportswirl: %i teleport texture(s) classified in %s\n", m5teleportcount, loadmodel->name);
	}
}
