...
void R_Modules_Start(void)
{
	...
	// METAL.md Phase 0: the renderer modules DO start on the Metal path, on
	// purpose. Skipping them was tried and reverted -- the texture POOLS live in
	// them (r_textures_start), so without them map and model loading dies at
	// "Mem_Alloc: pool == NULL" as soon as the startup demo loads a level.
	// Safety against the NULL qgl* pointers on this path (GL_InitFunctions is
	// only ever called by VID_InitModeGL) comes instead from sealing the two
	// doors into the renderer: CL_UpdateScreen's early-out and the guard at the
	// top of R_RenderView. A texture uploads only on bind and binds only happen
	// while rendering, so with both doors shut no GL call is reachable at all.
	for (i = 0;i < MAXRENDERMODULES;i++)
	{
		if (rendermodule[i].name == NULL)
			continue;
