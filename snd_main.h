...
// SEPTEMBER2 Part B: the reverb bus (snd_mix.c) and the mix dump (snd_main.c)
...
typedef struct channel_s
{
	...
	/// SEPTEMBER2 Part B (2026-09-06): set by the spatializer like volume[], read by the mixer.
	/// sendvol = this channel's level into the stereo reverb bus (0 = no send: music, local
	/// sounds, snd_reverb 0). lpalpha = the occlusion low-pass coefficient at the SFX's own
	/// rate (0 = bypass, the dry path untouched). lpstate is the filter's memory, mixer-only.
	float          sendvol;
	float          lpalpha;
	float          lpstate;
