...
/// Blend EQUATION, as opposed to the source/destination factors GL_BlendFunc
/// sets. ADD is the state everything else in the engine assumes, and every
/// caller must put it back.
///
/// The values are load-bearing: 0 and 1 are exactly the old
/// GL_BlendEquationSubtract(false) and (true), so the Metal pipeline key's byte
/// keeps its old meaning and a zeroed key is still ADD.
typedef enum dpblendop_e
{
	DPBLENDOP_ADD = 0,
	DPBLENDOP_REVSUBTRACT = 1,		///< dst - src; the shadow-volume counting mode
	DPBLENDOP_MAX = 2				///< max(src, dst) per channel; factors are ignored
}
dpblendop_t;
