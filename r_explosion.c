...
//	R_Mesh_ResetTextureState();
	...
	for (surfacelistindex = 0;surfacelistindex < numsurfaces;surfacelistindex++)
	{
		...
		// The shell is MODE_GENERIC and never had any fog term at all, and it is
		// drawn after the murk composite, so a rocket going off down a fogged
		// corridor used to read at full brightness through murk that hid the
		// corridor. Additive blend, so transmittance is a plain multiply -- no
		// tint term, an additive source adds less rather than turning grey.
		float murk = R_Volumetric_TransmittanceToPoint(e->origin, NULL);
		// FIXME: this can't properly handle r_refdef.view.colorscale > 1
		GL_Color(e->alpha * murk * r_refdef.view.colorscale, e->alpha * murk * r_refdef.view.colorscale, e->alpha * murk * r_refdef.view.colorscale, 1);
